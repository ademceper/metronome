...
export class WorkflowOverrideService {
  constructor(
    private config: { organizationId: string; environmentId: string }
  ) {}
  ...
  async createWorkflowOverride(
    override: Partial<ICreateWorkflowOverrideRequestDto> = {}
  ) {
    ...
    const workflowId =
      override.workflowId || (await this.createWorkflow(groups))._id
    ...
    const workflowOverride = await this.workflowOverrideRepository.create(
      payload as WorkflowOverrideEntity
    )
