...
export class SubscribersService {
  ...
  async createSubscriber(fields: Partial<SubscriberEntity> = {}) {
    ...
    const slackIntegration = integrations.find(
      (integration) => integration.providerId === ChatProviderIdEnum.Slack
    )
    const fcmIntegration = integrations.find(
      (integration) => integration.providerId === PushProviderIdEnum.FCM
    )
