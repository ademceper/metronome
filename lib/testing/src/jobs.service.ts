...
export class JobsService {
  public workflowQueue: Queue = new TestingQueueService(
    JobTopicNameEnum.WORKFLOW
  ).queue
  public standardQueue: Queue = new TestingQueueService(
    JobTopicNameEnum.STANDARD
  ).queue
  public subscriberProcessQueue: Queue = new TestingQueueService(
  ...
  public async waitForJobCompletion({
    ...
    const workflowMatch = templateId
      ? { _templateId: { $in: [templateId].flat() } }
      : {}
    const organizationMatch = organizationId
      ? { _organizationId: { $in: [organizationId].flat() } }
      : {}
  ...
  public async waitForDbJobCompletion({
    ...
    do {
      ...
      if (Date.now() - startTime > maxWaitTime) {
        throw new Error(
          `waitForDbJobCompletion timed out after ${maxWaitTime}ms. Mongo jobs: ${mongoJobsCount}`
        )
      }
  ...
  public async waitForWorkflowQueueCompletion(maxWaitTime = 10000) {
    return this.waitQueueUntil(
      ({ activeWorkflowJobsCount, waitingWorkflowJobsCount }) =>
        activeWorkflowJobsCount + waitingWorkflowJobsCount > 0,
      maxWaitTime
    )
  }
  ...
  public async waitForStandardQueueCompletion(maxWaitTime = 10000) {
    return this.waitQueueUntil(
      ({ activeStandardJobsCount, waitingStandardJobsCount }) =>
        activeStandardJobsCount + waitingStandardJobsCount > 0,
      maxWaitTime
    )
  }
  ...
  public async clearAllQueues() {
    try {
      await Promise.all([
        this.standardQueue.drain(),
        this.workflowQueue.drain(),
        this.subscriberProcessQueue.drain(),
      ])
    } catch (error) {
      console.warn(
        "Failed to clear Redis queues, continuing with test setup:",
        error
      )
    }
  }
  ...
  public async obliterateAllQueues() {
    try {
      ...
      console.warn(
        "Failed to obliterate Redis queues, continuing with test teardown:",
        error
      )
    }
  }
