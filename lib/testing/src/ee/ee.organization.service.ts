...
export class EEOrganizationService {
  private organizationRepository = getEERepository<OrganizationRepository>(
  ...
  async createOrganization(orgId: string) {
    ...
    const org = await this.communityOrganizationRepository.findOne({
      externalId: orgId,
    })
  ...
  async updateServiceLevel(
    ...
    await this.communityOrganizationRepository.update(
      { _id: organizationId },
