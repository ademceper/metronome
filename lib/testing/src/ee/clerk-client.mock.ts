...
export class ClerkClientMock {
  ...
  get users(): Partial<UserAPI> {
    ...
    const updateUserMetadata: UserAPI["updateUserMetadata"] = async (
      userId,
      params
    ) => {
      const user = this.getUserById(userId)
      ...
      const users = Array.from(this.clerkUsers.values()).filter((user) => {
        if (params.emailAddress && params.emailAddress.length > 0) {
          return user.emailAddresses.some(
            (emailAddress) =>
              emailAddress.emailAddress === params.emailAddress?.[0]
          )
        }
    ...
    const getOrganizationMembershipList: UserAPI["getOrganizationMembershipList"] =
      async (params) => {
        const users = Array.from(
          this.clerkOrganizationMemberships.values()
        ).filter((membership) => membership.organization.id === params.userId)
  ...
  get organizations() {
    const getOrganization: OrganizationAPI["getOrganization"] = async (
      ...
      if ("organizationId" in params) {
        ...
        if (!org)
          throw new Error(
            `Organization not found with id ${params.organizationId}`
          )
      ...
      if ("slug" in params) {
        const org = Array.from(this.clerkOrganizations.values()).find(
          (_org) => _org.slug === params.slug
        )
        if (!org)
          throw new Error(`Organization not found with slug ${params.slug}`)

        return org
      }

      throw new Error(
        "Invalid parameters: must provide either organizationId or slug"
      )
    }

    const getOrganizationMembershipList: OrganizationAPI["getOrganizationMembershipList"] =
      async (params) => {
    ...
    const createOrganizationMembership: OrganizationAPI["createOrganizationMembership"] =
      async (params) => {
        const newMembership = {
          ...params,
          createdAt: Date.now(),
    ...
    const updateOrganization: OrganizationAPI["updateOrganization"] = async (
      organizationId,
      params
    ) => {
    ...
    const updateOrganizationMetadata: OrganizationAPI["updateOrganizationMetadata"] =
    ...
    const deleteOrganization: OrganizationAPI["deleteOrganization"] = async (
      organizationId
    ) => {
      const organization = this.clerkOrganizations.get(organizationId)
      if (!organization)
