...
import type { IApiRateLimitMaximum } from "@novu/shared"
...
export class EnvironmentService {
  ...
  async getEnvironments(organizationId: string): Promise<EnvironmentEntity[]> {
    return await this.environmentRepository.findOrganizationEnvironments(
      organizationId
    )
  }
  ...
  async getDevelopmentEnvironment(
    ...
    return await this.getEnvironmentByNameAndOrganization(
      organizationId,
      EnvironmentsEnum.DEVELOPMENT
    )
  }

  async getProductionEnvironment(
    organizationId: string
  ): Promise<EnvironmentEntity | undefined> {
    return await this.getEnvironmentByNameAndOrganization(
      organizationId,
  ...
  async updateApiRateLimits(
    environmentId: string,
    apiRateLimits: Partial<IApiRateLimitMaximum>
  ) {
    return await this.environmentRepository.updateApiRateLimits(
      environmentId,
      apiRateLimits
    )
  }
