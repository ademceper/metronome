...
export function sanitizeRedirect(redirect: InAppRedirectType | undefined) {
  ...
  return {
    url,
    target: (redirect.target ?? defaultTarget) as
      | "_self"
      | "_blank"
      | "_parent"
      | "_top"
        ...
        if (Array.isArray(value)) {
          result[key as keyof T] = value.map((item) =>
            typeof item === "object" && item !== null
              ? filterNullishValues(item as Record<string, unknown>)
              : item
          ) as T[keyof T]
        } else if (typeof value === "object") {
          result[key as keyof T] = filterNullishValues(
...
function isTimedDelayControl(
  controlValues: unknown
): controlValues is DelayTimedControlType {
  return !isEmpty((controlValues as DelayTimedControlType)?.cron)
}

function isDynamicDelayControl(
  controlValues: unknown
): controlValues is DelayDynamicControlType {
  return !isEmpty((controlValues as DelayDynamicControlType)?.dynamicKey)
}

function isRegularDelayControl(
  controlValues: unknown
): controlValues is DelayRegularControlType {
  return (
    !isTimedDelayControl(controlValues) && !isDynamicDelayControl(controlValues)
  )
}
