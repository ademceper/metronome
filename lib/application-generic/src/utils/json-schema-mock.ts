...
export class JsonSchemaMock {
   ...
   */
  private static getExampleValueForStringProperty(
    key: string,
    ...
    // Color patterns
    if (
      JsonSchemaMock.matchesPattern(lowerKey, ["color", "colour", "hex", "rgb"])
    ) {
      return "#3B82F6"
    }
  ...
  private static addExamplesToSchema(schema: any): any {
    ...
    if (enhancedSchema.type) {
      enhancedSchema.type = JsonSchemaMock.normalizeTypeForExamples(
        enhancedSchema.type
      )
    }
      ...
      for (const [key, propertySchema] of Object.entries(
        ...
        if (propertySchema && typeof propertySchema === "object") {
          ...
          if (
            ...
            prop.examples = [
              JsonSchemaMock.getExampleValueForStringProperty(key, prop),
            ]
          }
          ...
          if (effectiveType === "object" || prop.properties) {
            ...
            enhancedSchema.properties[key] =
              JsonSchemaMock.addExamplesToSchema(prop)
          }
          ...
          if (effectiveType === "array" && prop.items) {
            ...
            if (
              ...
              prop.items.type = JsonSchemaMock.normalizeTypeForExamples(
                prop.items.type
              )
            }
