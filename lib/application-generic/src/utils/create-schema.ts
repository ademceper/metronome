  ...
  return {
    ...
    properties: {
      ...
      data: buildVariablesSchema(
        subscriber && typeof subscriber === "object" && "data" in subscriber
          ? subscriber.data
          : {}
      ),
    },
...
export const buildWorkflowSchema = () => {
  return {
    ...
    properties: {
      ...
      tags: {
        type: JsonSchemaTypeEnum.ARRAY,
        items: { type: JsonSchemaTypeEnum.STRING },
      },
      severity: {
        type: JsonSchemaTypeEnum.STRING,
...
export const buildContextSchema = (context?: unknown) => {
  const baseSchema = {
    type: JsonSchemaTypeEnum.OBJECT,
    description:
      "Context data passed at trigger time following ContextPayload structure",
