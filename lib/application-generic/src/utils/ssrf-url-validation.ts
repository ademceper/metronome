...
export class SsrfBlockedError extends Error {
  ...
  constructor(
    ...
    message: string,
    extra?: { resolvedAddress?: string; hostname?: string }
  ) {
    super(message)
...
export async function resolvePublicAddresses(
  ...
  if (!addresses) {
    try {
      ...
      throw new SsrfBlockedError(
        "DNS_LOOKUP_FAILED",
        `Unable to resolve hostname "${lower}".`,
        { hostname: lower }
      )
    }
...
export type SafeOutboundMethod =
  | "GET"
  | "POST"
  | "PUT"
  | "DELETE"
...
async function resolveWithTestAllowList(
  hostname: string
): Promise<dns.LookupAddress[]> {
...
export async function safeOutboundRequest(
  ...
  for (let redirect = 0; redirect <= maxRedirects; redirect += 1) {
    ...
    if (
      redirect > 0 &&
      initialOriginHost &&
      parsed.host.toLowerCase() !== initialOriginHost
    ) {
    ...
    if (REDIRECT_STATUS_CODES.has(status) && redirect < maxRedirects) {
      ...
      if (
        ...
        initialOriginHost &&
        nextUrl.host.toLowerCase() !== initialOriginHost
      ) {
        throw new SsrfBlockedError(
          "CROSS_ORIGIN_METHOD_PRESERVING_REDIRECT",
...
export async function safeOutboundJsonRequest<T = unknown>(
  ...
  const body = isPlainObject
    ...
    : (options.body as string | Buffer | Readable | undefined)
