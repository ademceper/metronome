import type { JSONContent as MailyJSONContent } from "@novu/maily-render"
import { TRANSLATION_KEY_SINGLE_REGEX } from "@novu/shared"
import {
  MAILY_FIRST_CITIZEN_VARIABLE_KEY,
  MailyAttrsEnum,
  MailyContentTypeEnum,
} from "../types/maily.types"

export const isStringifiedMailyJSONContent = (
  value: unknown
): value is string => {
...
export const isObjectMailyJSONContent = (
  value: unknown
): value is MailyJSONContent => {
...
): node is MailyJSONContent & {
  attrs: { [MailyAttrsEnum.EACH_KEY]: string }
} => {
  return !!(
...
export const isButtonNode = (
  ...
  return !!(
    ...
    ((node.attrs[MailyAttrsEnum.TEXT] !== undefined &&
      ...
      (node.attrs[MailyAttrsEnum.URL] !== undefined &&
        typeof node.attrs[MailyAttrsEnum.URL] === "string"))
  )
}
...
export const isImageNode = (
  ...
  return !!(
    (node.type === MailyContentTypeEnum.IMAGE ||
      node.type === MailyContentTypeEnum.INLINE_IMAGE) &&
    node.attrs &&
    ((node.attrs[MailyAttrsEnum.SRC] !== undefined &&
      typeof node.attrs[MailyAttrsEnum.SRC] === "string") ||
...
export const hasShow = (
  ...
  attrs: { [MailyAttrsEnum.SHOW_IF_KEY]: string }
} => {
  return (
    node.attrs?.[MailyAttrsEnum.SHOW_IF_KEY] !== undefined &&
    node.attrs?.[MailyAttrsEnum.SHOW_IF_KEY] !== null
  )
}

export const hasAttrs = (
  node: MailyJSONContent
): node is MailyJSONContent & { attrs: Record<string, any> } => {
  return !!node.attrs
}

export const hasMarks = (
  node: MailyJSONContent
): node is MailyJSONContent & { marks: Record<string, any>[] } => {
  return !!node.marks
}
...
type ProcessAttributesFunction = (
  args: ProcessAttributesArgs
...
const processMailyNodes = ({
  ...
  const newNode = { ...node } as MailyJSONContent & {
    attrs: Record<string, any>
  }
...
export const replaceMailyNodesByCondition = (
  ...
  replacementFn: (
    node: MailyJSONContent
  ) => MailyJSONContent | MailyJSONContent[] | null
): MailyJSONContent => {
 ...
 * },
 */
export const replaceMailyVariables = (
  content: string,
...
export const wrapMailyInLiquid = (content: string) => {
  ...
  return processMailyNodes({
    ...
    shouldProcessFlag: ({ flagKey }) =>
      !MAILY_FIRST_CITIZEN_VARIABLE_KEY.includes(flagKey),
    processFlag: () => {
      return false
    },
  })
