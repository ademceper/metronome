...
function isPropertyAllowed(
  schema: JSONSchemaDto | undefined,
  propertyPath: string
) {
  if (!schema) {
    return true
...
function validateVariable({
  ...
  const isNotStepVariable =
    ...
    !DIGEST_EVENTS_VARIABLE_PATTERN.test(variableName)
  const isLocalVariable = Array.from(localVariables).some(
    (localVar) =>
      variableName === localVar || variableName.startsWith(`${localVar}.`)
  )

  const isAllowedVariable = isPropertyAllowed(variableSchema, variableName)
  const isContentVariable =
    variableName === LAYOUT_CONTENT_VARIABLE && isAllowedVariable
  const isTranslationVariable = variableName.startsWith(
    TRANSLATION_NAMESPACE_SEPARATOR
  )
  ...
  if (
    (hasNoNamespace && !isContentVariable) ||
    (!variableSchema && isNotStepVariable)
  ) {
      ...
      if (
        ...
        if (
          (isValidDynamicPath(variableName) &&
            isPropertyAllowed(
              variableSchema,
              `${variableName}.${firstArg.content}`
            )) ||
          (firstArg.content.startsWith("payload.") &&
            DIGEST_EVENTS_VARIABLE_PATTERN.test(variableName))
        ) {
          const isFirstArgValid = isPropertyAllowed(
            variableSchema,
            firstArg.content
          )
          if (isFirstArgValid) {
...
function processTagToken({
  ...
  if (template instanceof ForTag) {
    ...
    if (forMatch) {
      ...
      const isRangeExpression = collectionExpression
        .trim()
        .match(/^\(.+?\.\..+?\)$/)
      ...
      if (isRangeExpression) {
        ...
        const rangeVariables = extractVariablesFromRange(
          collectionExpression.trim()
        )
        ...
        const collectionVariable =
          extractVariableFromExpression(collectionExpression)
    ...
    const conditionMatch = output.match(
      new RegExp(`^\\s*{%\\s*${tagName}\\s+(.+?)\\s*%}`)
    )
    if (conditionMatch) {
    ...
    const assignMatch = output.match(
      /^\s*{%-?\s*assign\s+(\w+)\s*=\s*(.+?)\s*-?%}/
    )
    if (assignMatch) {
    ...
    const tablerowMatch = output.match(
      /^\s*{%\s*tablerow\s+(\w+)\s+in\s+(.+?)\s*%}/
    )
    if (tablerowMatch) {
    ...
    for (const branch of branches) {
      ...
      if (branch.values) {
        branch.values.forEach((valueToken: any) => {
          ...
          if (
            valueToken.kind === TokenKind.PropertyAccess ||
            valueToken.kind === TokenKind.Word
          ) {
            const variableName = valueToken.input
              .slice(valueToken.begin, valueToken.end)
              .trim()
...
function extractVariablesFromCondition(condition: string): string[] {
  ...
  const variableMatches = processedCondition.match(
...
function extractProps(template: any): {
  valid: boolean
  props: string[]
  error?: string
} {
...
function validateFilters(
  filters: Filter[],
  isDigestEventsVariable: boolean
): LiquidFilterIssue[] {
  ...
  for (const tmpl of templates) {
    ...
    if (isTagToken(tmpl)) {
      if (tmpl instanceof ForTag || tmpl instanceof TablerowTag) {
        ...
        const loopMatch = tagOutput.match(
          new RegExp(`^\\s*{%\\s*${tagName}\\s+\\w+\\s+in\\s+(.+?)\\s*%}`)
        )
        if (loopMatch) {
...
function annotateCollectionVariable(
  ...
  const iteratorProps = [
    ...new Set(
      rawNames
        .filter((name) => name.startsWith(iteratorPrefix))
        .map((name) => name.slice(iteratorPrefix.length))
    ),
  ]
