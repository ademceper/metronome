...
export const DIGEST_EVENTS_PAYLOAD_VARIABLE_PATTERN =
...
export function isValidDynamicPath(variableName: string): boolean {
  return VALID_DYNAMIC_PATHS.some((path) =>
    typeof path === "string"
      ? variableName.startsWith(path)
      : path.test(variableName)
  )
}

export function isLiquidErrors(error: unknown): error is LiquidErrors {
  return (
    error instanceof LiquidError &&
    "errors" in error &&
    Array.isArray((error as LiquidErrors).errors)
  )
}
