...
export function buildVariables({
  ...
  if (isStringifiedMailyJSONContent(variableControlValue)) {
    ...
    isValidRule(variableControlValue as RulesLogic<AdditionalOperation>)
  ) {
    const fields = extractFieldsFromRules(
      variableControlValue as RulesLogic<AdditionalOperation>
    )
      .filter(
        (field) =>
          field.startsWith("payload.") ||
          field.startsWith("subscriber.data.") ||
  ...
  const { validVariables, invalidVariables } =
    newExtractLiquidTemplateVariables({
      template:
        typeof variableControlValue === "string"
          ? variableControlValue
          : JSON.stringify(variableControlValue),
      variableSchema,
