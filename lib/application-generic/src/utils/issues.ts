...
const mapAjvErrorToMessage = (
  ...
  if (
    isUrlField &&
    (error.keyword === "anyOf" || error.keyword === "pattern")
  ) {
...
const validateContentCompilation = (
  controlKey: string,
  ...
  try {
    ...
    const message = error.message
      ? error.message.split(", line:1")[0] || error.message.split(" line:1")[0]
      : ""
...
export const processControlValuesByLiquid = ({
  ...
  if (!currentValue || typeof currentValue !== "object") {
    ...
    if (liquidTemplateIssues.invalidVariables.length > 0) {
      ...
      issues.controls[controlKey] = liquidTemplateIssues.invalidVariables.map(
        (invalidVariable) => {
          const message = invalidVariable.message
            ? invalidVariable.message.split(" line:")[0]
            : ""
          const variableName =
            invalidVariable.name === "unknown" ? "{{}}" : invalidVariable.name
      ...
      const contentIssue = validateContentCompilation(
        contentControlKey,
        currentValue
      )
      if (contentIssue) {
