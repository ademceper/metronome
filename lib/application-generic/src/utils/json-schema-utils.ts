 ...
 * // }
 */
export function mockSchemaDefaults(
...
export function keysToObject(
  ...
  return buildObjectFromPaths(
    validPaths,
    arrayVariables || [],
    showIfVariablesPaths || []
  )
}
...
function buildObjectFromPaths(
  ...
  sortedPaths.forEach((path) => {
    const digestEventsMatch = path.match(
  ...
  sortedPaths.forEach((path) => {
    ...
    if (
      lastPart === "payload" &&
      DIGEST_EVENTS_VARIABLE_PATTERN.test(finalPart)
    ) {
      /*
    ...
    const arrayParent = arrayVariables.find(
      (arrayVariable) =>
        arrayVariable.path === path || path.startsWith(`${arrayVariable.path}.`)
    )
    if (!arrayParent) {
    ...
    const targetPath = isDirectArrayPath
      ? path
      : `${arrayParent.path}[0].${path.slice(arrayParent.path.length + 1)}`
...
export function mergeCommonObjectKeys(
  ...
  if (Array.isArray(source) && Array.isArray(target)) {
    const mergedArray = source.map((sItem, i) => {
      ...
      return mergeCommonObjectKeys(
        tItem as Record<string, unknown>,
        sItem as Record<string, unknown>
      )
    })
    ...
    if (mergedArray.length < target.length) {
      ...
      for (let idx = 0; idx < fillCount; idx += 1) {
        const mergedObject = mergeCommonObjectKeys(
          remainingItems[idx],
          lastItem
        )
  ...
  for (const key of Object.keys(source)) {
    ...
    if (tVal !== undefined && tVal !== null) {
      result[key] = mergeCommonObjectKeys(
