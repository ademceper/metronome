...
export { getLoggerToken, Logger, LoggerModule, PinoLogger, Store, storage }
...
export function getLogLevel() {
  ...
  if (process.env.LOGGING_LEVEL || process.env.LOG_LEVEL) {
    ...
    console.log(
      `Environment variable LOG_LEVEL is not set. Falling back to info level.`
    )
    logLevel = "info"
  }
  ...
  if (!loggingLevelArr.includes(logLevel)) {
    console.log(
      `${logLevel}is not a valid log level of ${loggingLevelArr}. Falling back to info level.`
    )
...
export function createNestLoggingModuleOptions(settings: {
  ...
  return {
    exclude: [
      ...
      {
        path: "/v1/internal/subscriber-online-state",
