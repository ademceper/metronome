...
export class SendWebhookMessage {
  ...
  @InstrumentUsecase()
  async execute(
    command: SendWebhookMessageCommand
  ): Promise<{ eventId: string } | undefined> {
    if (!this.svix) {
      this.logger.debug(
        "Outbound webhook client not available – webhooks are disabled for this instance."
      )
    ...
    if (!appId) {
      this.logger.debug(
        `Webhook app ID not found for environment ${command.environmentId}`
      )
    ...
    try {
      ...
      this.logger.debug(
        `Successfully sent webhook ${command.eventType}. Event ID: ${eventId}`
      )
