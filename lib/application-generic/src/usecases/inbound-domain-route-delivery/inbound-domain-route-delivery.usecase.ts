...
export class InboundDomainRouteDelivery {
  ...
  async deliverToAgent(params: {
    ...
    this.logger.info(
      ...
      "Delivering inbound email to agent"
    )
    ...
    const { identifier: integrationIdentifier, secretKey } =
      await this.resolveIntegration(
        agentId,
    ...
    if (!apiBaseUrl) {
      this.throwError(
        "API_ROOT_URL environment variable is not set — cannot forward inbound email to agent webhook"
      )
    }
  ...
  private coerceToBuffer(value: unknown): Buffer | null {
    if (Buffer.isBuffer(value)) {
      if (value.length > MAX_ATTACHMENT_BYTES) {
        this.logger.warn(
          { size: value.length, cap: MAX_ATTACHMENT_BYTES },
          "Attachment exceeds cap; skipping"
        )
  ...
  private mapAttachmentsForWebhook(
    ...
    const result = rawAttachments.map((a) => {
      ...
      const filename =
        att.fileName ?? att.generatedFileName ?? att.filename ?? "attachment"
  ...
  private async resolveIntegration(
    ...
    if (!integration) {
      this.throwError(
        `No active NovuAgent email integration found for agent ${agentId}`
      )
    }
    ...
    return {
      ...
      secretKey: decryptSecret(encryptedSecret),
    }
  }
