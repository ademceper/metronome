...
type SelectedControlValuesFields = Pick<
  ControlValuesEntity,
  "controls" | "_stepId"
>

const SELECTED_CONTROL_VALUES_PROJECTION: Record<
  keyof SelectedControlValuesFields,
  1
> & { _id: 0 } = {
  controls: 1,
  _stepId: 1,
...
export class BuildVariableSchemaUsecase {
  ...
  async execute(command: BuildVariableSchemaCommand): Promise<JSONSchemaDto> {
    ...
    const optimisticControlValues = Object.values(
      command.optimisticControlValues || {}
    )
    const { payload, subscriber, context } =
      await this.createVariablesObject.execute(
        CreateVariablesObjectCommand.create({
          environmentId: command.environmentId,
          organizationId: command.organizationId,
          controlValues:
            optimisticControlValues.length > 0
              ? optimisticControlValues
              : workflowControlValues,
        })
      )
    ...
    const previousSteps = effectiveSteps?.slice(
      0,
      this.findStepIndex(effectiveSteps, stepInternalId)
    )

    const effectivePayloadSchema =
      optimisticPayloadSchema ?? workflow?.payloadSchema
    ...
    return {
      ...
      properties: {
        ...
        payload: await this.resolvePayloadSchema(
          workflow,
          finalPayload,
          optimisticPayloadSchema
        ),
  ...
  private buildEffectiveSteps(
    ...
    const existingStepIds = new Set(
      existingSteps.map((step) => step.stepId).filter(Boolean)
    )

    // Add optimistic steps that don't already exist
    const newOptimisticSteps = optimisticSteps.filter(
  ...
  private findStepIndex(
    effectiveSteps:
      | Array<NotificationStepEntity | IOptimisticStepInfo>
      | undefined,
  ...
  private async resolvePayloadSchema(
    ...
    if (optimisticPayloadSchema) {
      return (
        parsePayloadSchema(optimisticPayloadSchema, { safe: true }) ||
        emptyJsonSchema()
      )
    }
  ...
  private mergePreviewData(
    ...
    return {
      ...
      subscriber: {
        ...((extracted.subscriber as Record<string, unknown>) || {}),
        ...(previewData?.subscriber || {}),
      },
      context: {
        ...((extracted.context as Record<string, unknown>) || {}),
        ...(previewData?.context || {}),
      },
    }
  }
      ...
      if ("template" in step && step.template?.type) {
        ...
        if (
          ...
          if (stepControls?.responseBodySchema) {
            responseBodySchema =
              stepControls.responseBodySchema as JSONSchemaDto
          }
        }
