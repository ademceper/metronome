...
export class CreateWorkflowV0 {
  ...
  @InstrumentUsecase()
  async execute(
    usecaseCommand: CreateWorkflowCommandV0
  ...
  private generateTriggerIdentifier(command: CreateWorkflowCommandV0) {
    ...
    if (
      command.type === ResourceTypeEnum.BRIDGE &&
      command.origin === ResourceOriginEnum.EXTERNAL
    )
      /*
  ...
  private async storeWorkflow(
    ...
    const workflowData = {
      ...
      ...(command.payloadSchema
        ...
        : {}),
      ...(command.validatePayload !== undefined
        ? { validatePayload: command.validatePayload }
        : {}),
  ...
  private async storeTemplateSteps(
    ...
    for (const step of command.steps) {
      if (!step.template)
        throw new BadRequestException(
          `Unexpected error: message template is missing`
        )
