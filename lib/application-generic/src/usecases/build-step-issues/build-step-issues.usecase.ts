...
export class BuildStepIssuesUsecase {
  ...
  async execute(command: BuildStepIssuesCommand): Promise<StepIssuesDto> {
    ...
    const variableSchema =
      await this.buildAvailableVariableSchemaUsecase.execute(
        BuildVariableSchemaCommand.create({
          ...
          workflow: persistedWorkflow,
          ...(controlValuesDto
            ? { optimisticControlValues: controlValuesDto }
            : {}),
          ...(command.optimisticSteps
            ? { optimisticSteps: command.optimisticSteps }
            : {}),
    ...
    const isStepResolverStep = this.isStepResolverStep(
      persistedWorkflow,
      stepInternalId
    )
    ...
    const skipLogicIssues = sanitizedControlValues?.skip
      ? this.validateSkipField(
          variableSchema,
          sanitizedControlValues.skip as RulesLogic<AdditionalOperation>
        )
      : {}
  ...
  private isStepResolverStep(
    persistedWorkflow?: BuildStepIssuesCommand["workflow"],
    ...
    const currentStep = persistedWorkflow.steps.find(
      (step) =>
        step._id === stepInternalId || step._templateId === stepInternalId
    )
  ...
  private async processControlValuesByCustomeRules(
    ...
    const restrictionsErrors =
      await this.tierRestrictionsValidateUsecase.execute(
    ...
    if (Array.isArray(obj)) {
      return obj.map((item) => {
        ...
        if (typeof item === "object" && item !== null) {
          return this.frameworkSanitizeEmptyStringsToNull(
    ...
    return Object.fromEntries(
      Object.entries(obj).map(([key, value]) => {
        ...
        if (typeof value === "object" && value !== null) {
          return [
            key,
            this.frameworkSanitizeEmptyStringsToNull(
  ...
  @Instrument()
  private validateSkipField(
    variableSchema: JSONSchemaDto,
    skipLogic: RulesLogic<AdditionalOperation>
  ): StepIssuesDto {
