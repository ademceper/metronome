...
export class ExecuteFrameworkRequest {
  ...
  async execute<T extends PostActionEnum | GetActionEnum>(
    ...
    const timeout = bridgeUrl?.includes(process.env.API_INTERNAL_ORIGIN)
      ? 60_000
      : DEFAULT_TIMEOUT
    const method = [PostActionEnum.EXECUTE, PostActionEnum.PREVIEW].includes(
    ...
    try {
      ...
      >({
        ...
        onRetry: ({ statusCode, errorCode, delay }) => {
          if (statusCode) {
            this.logger.info(
              `Retryable status code ${statusCode} detected. Retrying in ${delay}ms`
            )
          } else if (errorCode) {
            this.logger.info(
              `Retryable error code ${errorCode} detected. Retrying in ${delay}ms`
            )
          }
        },
      })
  ...
  private handleHttpStatusError(
    statusCode: number,
    url: string
  ): Pick<BridgeError, "code" | "statusCode" | "message"> {
    switch (statusCode) {
      case 401:
        return {
          message:
            BRIDGE_EXECUTION_ERROR.BRIDGE_AUTHENTICATION_FAILED.message(url),
  ...
  private async handleResponseError(
    ...
    if (!(error instanceof HttpClientError)) {
      ...
      if (
        error.type === HttpClientErrorType.HTTP_ERROR &&
        isFrameworkError(body)
      ) {
        bridgeErrorData = {
          data: body.data,
          code: body.code,
        ...
        switch (error.type) {
          case HttpClientErrorType.TIMEOUT:
            ...
            bridgeErrorData = {
              ...
              message:
                BRIDGE_EXECUTION_ERROR.BRIDGE_REQUEST_TIMEOUT.message(url),
          ...
          case HttpClientErrorType.SSRF_BLOCKED:
            ...
            this.logger.warn(
              { err: error },
              `Blocked outbound bridge request to \`${url}\``
            )
            bridgeErrorData = {
              code: error.networkCode || "SSRF_BLOCKED",
              message:
                "The provided bridge URL is blocked by the outbound SSRF policy.",
              statusCode: HttpStatus.BAD_REQUEST,
            }
            break
          ...
          case HttpClientErrorType.CERTIFICATE_ERROR:
            bridgeErrorData = {
              ...
              message:
                BRIDGE_EXECUTION_ERROR.SELF_SIGNED_CERTIFICATE.message(url),
              statusCode: HttpStatus.BAD_REQUEST,
            }
            break
          ...
          case HttpClientErrorType.NETWORK_ERROR:
            if (
              error.networkCode &&
              RETRYABLE_ERROR_CODES.includes(error.networkCode)
            ) {
              bridgeErrorData = {
                message:
                  BRIDGE_EXECUTION_ERROR.BRIDGE_ENDPOINT_UNAVAILABLE.message(
                    url
                  ),
              ...
              bridgeErrorData = {
                message:
                  BRIDGE_EXECUTION_ERROR.UNKNOWN_BRIDGE_REQUEST_ERROR.message(
                    url
                  ),
                code: BRIDGE_EXECUTION_ERROR.UNKNOWN_BRIDGE_REQUEST_ERROR.code,
                statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
          ...
          case HttpClientErrorType.HTTP_ERROR: {
            if (body?.code === TUNNEL_ERROR_CODE) {
              ...
              bridgeErrorData = this.handleHttpStatusError(
                error.statusCode,
                url
              )
