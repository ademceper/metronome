...
export class TriggerMulticast extends TriggerBase {
  ...
  async execute(command: TriggerMulticastCommand) {
    ...
    try {
      ...
      const { singleSubscribers, topicKeys, topicExclusions } =
      ...
      const allTopicExcludedSubscribers = Array.from(
        new Set([
          ...Array.from(topicExclusions.values()).flatMap((set) =>
            Array.from(set)
          ),
        ])
      )
      ...
      const contextKeysForQuery = useContextFiltering
        ? command.contextKeys
        : undefined

      const getTopicDistinctSubscribersGenerator =
        this.topicSubscribersRepository.getTopicDistinctSubscribers({
          query: {
      ...
      for await (const subscription of getTopicDistinctSubscribersGenerator) {
        ...
        if (existingSubscriber) {
          if (
            !existingSubscriber.topics.some(
              (t) => t.subscriptionIdentifier === subscriptionId
            )
          ) {
      ...
      if (isTopic(recipient)) {
        ...
        if (topicRecipient.exclude && topicRecipient.exclude.length > 0) {
          const existingExclusions =
            acc.topicExclusions.get(topicRecipient.topicKey) ||
            new Set<string>()
