...
export class TriggerEvent {
  ...
  async execute(command: TriggerEventCommand) {
    ...
    try {
      ...
      const mappedCommand = await this.getMappedCommand(
        command,
        storedWorkflow?._id
      )
  ...
  private async getMappedCommand(
    ...
    if (workflow) {
      ...
      if (isBackendSDK) {
        ...
        const shouldUpdate =
          ...
          new Date(workflow.lastTriggeredAt).getTime() <
            lastTriggeredAt.getTime() - 5 * 60 * 1000

        if (shouldUpdate) {
          const previousLastTriggeredAt = workflow.lastTriggeredAt
  ...
  private async findWorkflowByTriggerIdentifier(
    ...
    return this.inMemoryLRUCacheService.get(
      ...
      () =>
        this.notificationTemplateRepository.findByTriggerIdentifier(
          environmentId,
          triggerIdentifier
        ),
      {
  ...
  private async resolveContextKeys(
    command: TriggerEventCommand,
    workflowId: string
  ): Promise<string[]> {
    if (!command.context) {
    ...
    try {
      const contexts =
        await this.contextRepository.findOrCreateContextsFromPayload(
          command.environmentId,
          command.organizationId,
          command.context
        )
