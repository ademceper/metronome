...
export class GetPreferences {
  ...
  private async getPreferencesFromDb(
    ...
    if (command.subscriberId) {
      ...
      const contextQuery =
        this.preferencesRepository.buildContextExactMatchQuery(
          command.contextKeys,
          {
    ...
    if (workflowResourcePreference) {
      result.workflowResourcePreference =
        workflowResourcePreference as PreferenceSet["workflowResourcePreference"]
    }

    if (workflowUserPreference) {
      result.workflowUserPreference =
        workflowUserPreference as PreferenceSet["workflowUserPreference"]
    }
  ...
  @Instrument()
  private async getPreferencesFromDbOptimized(
    command: GetPreferencesCommand
  ): Promise<PreferenceSet> {
    const baseQuery = {
      _environmentId: command.environmentId,
    ...
    if (command.templateId) {
      const workflowPreferences = await this.inMemoryLRUCacheService.get(
        ...
        async (): Promise<
          [PreferencesEntity | null, PreferencesEntity | null]
        > => {
          ...
          const workflowResourcePref =
            preferences.find(
              (p) => p.type === PreferencesTypeEnum.WORKFLOW_RESOURCE
            ) ?? null
          const workflowUserPref =
            preferences.find(
              (p) => p.type === PreferencesTypeEnum.USER_WORKFLOW
            ) ?? null
      ...
      ;[workflowResourcePreference, workflowUserPreference] =
        workflowPreferences
    }
