...
export class UpsertPreferences {
  ...
  @Instrument()
  public async upsertTopicSubscriptionPreferences(
    command: UpsertSubscriberWorkflowPreferencesCommand
  ) {
    return this.upsert({
      _subscriberId: command._subscriberId,
  ...
  private async createPreferences(
    ...
    try {
      return await this.preferencesRepository.create({
        ...
        schedule: command.schedule,
        contextKeys:
          useContextFiltering && isContextScoped
            ? (command.contextKeys ?? [])
            : undefined,
      })
    } catch (error) {
  ...
  private async getPreference(
    ...
    const contextQuery = this.preferencesRepository.buildContextExactMatchQuery(
      command.contextKeys,
      {
