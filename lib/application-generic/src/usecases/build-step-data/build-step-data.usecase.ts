...
export class BuildStepDataUsecase {
  ...
  async execute(
    ...
    const currentStep: NotificationStepEntity | undefined =
      await this.loadStepsFromDb(command, workflow)

    if (!currentStep || !currentStep._templateId) {
      throw new InvalidStepException(command.stepIdOrInternalId)
    }

    const controlValues = await this.getControlValues(
      command,
      currentStep,
      workflow._id
    )
  ...
  static mapToStepResponse(
    ...
    const stepName =
      currentStep.name || "MISSING STEP NAME - PLEASE UPDATE IMMEDIATELY"
    const slug = buildSlug(
      stepName,
      ShortIsPrefixEnum.STEP,
  ...
  private async loadStepsFromDb(
    ...
    const currentStep: NotificationStepEntity | undefined = workflow.steps.find(
      (stepItem) =>
        stepItem._id === command.stepIdOrInternalId ||
