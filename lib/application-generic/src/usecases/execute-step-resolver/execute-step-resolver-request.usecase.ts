...
export class ExecuteStepResolverRequest {
  ...
  async execute(command: ExecuteBridgeRequestCommand): Promise<ExecuteOutput> {
    ...
    if (!dispatchUrl) {
      throw new NotFoundException(
        "Step resolver dispatch URL is not configured"
      )
    }
    ...
    if (!command.organizationId) {
      throw new NotFoundException(
        "organizationId is required for Step Resolver"
      )
    }
    ...
    const compiledEvent = await this.compileControlValues(
      normalizedEvent,
      url,
  ...
  private transformToExecuteOutput(
    response: StepResolverResponse,
    duration: number
  ): ExecuteOutput {
    return {
  ...
  private async compileControlValues(
    ...
    try {
      ...
      const stateArray = Array.isArray(event.state)
        ? (event.state as State[])
        : []
      const stepsMap = stateArray.reduce<
      ...
      const compiledString = await liquidEngine.render(
        parsedTemplate,
        renderVariables
      )
  ...
  private async handleResponseError(
    ...
    const stepResolverError = this.buildErrorResponse(
      error,
      url,
      stepResolverHash
    )
  ...
  private buildErrorResponse(
    ...
    if (error instanceof HTTPError) {
          ...
          return {
            ...
            message:
              typeof parsedBody.message === "string"
                ? parsedBody.message
                : "Step controls failed schema validation",
            statusCode,
        ...
        if (parsedBody?.error === "STEP_HANDLER_ERROR") {
          return {
            ...
            message:
              typeof parsedBody.message === "string"
                ? parsedBody.message
                : "An error occurred in your template code",
            statusCode,
            cause: error,
          }
        }
      ...
      if (statusCode >= 500) {
        this.logger.error(
          { error, statusCode, url, stepResolverHash },
          `Step resolver HTTP error: ${statusCode}`
        )
      }
      ...
      const message =
        mapping?.message ?? `Step resolver returned status ${statusCode}`
    ...
    this.logger.error(
      { error, url, stepResolverHash },
      `Step resolver request failed: ${url}`
    )
    ...
    return {
      ...
      message: isTimeout
        ? `Step resolver request timeout: ${url}`
        : `Step resolver request failed: ${url}`,
