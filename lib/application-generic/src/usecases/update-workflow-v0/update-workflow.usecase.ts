...
export class UpdateWorkflowV0 {
  ...
  async execute(
    ...
    const existingTemplate: WorkflowWithPreferencesResponseDto =
    ...
    const allowedTemplateIds = this.buildAllowedTemplateIds(
    ...
    if (command.session) {
      ...
      await this.notificationTemplateRepository.withTransaction(
        async (session) => {
  ...
  private assertTemplateBelongsToWorkflow(
    ...
    if (!allowedTemplateIds.has(templateId.toString())) {
      throw new BadRequestException(
        `Template ${templateId} does not belong to this workflow`
      )
    }
  }
  ...
  private async updateMessageTemplates(
    ...
    for (const message of steps) {
      ...
      this.assertTemplateBelongsToWorkflow(
        message._templateId,
        allowedTemplateIds
      )
