...
import type { GetEnvironmentTagsDto } from "../../dtos/get-environment-tags.dto"
import type { GetEnvironmentTagsCommand } from "./get-environment-tags.command"
...
export class GetEnvironmentTags {
  ...
  private async resolveEnvironment(
    ...
    const isInternalId = BaseRepository.isInternalId(
    ...
    if (isInternalId) {
      ...
      const environment =
        await this.environmentRepository.findEnvironmentByIdentifier(
          command.environmentIdOrIdentifier
        )

      if (
        environment &&
        environment._organizationId === command.organizationId
      ) {
        return environment
      }
