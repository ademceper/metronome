...
export class UpsertWorkflowUseCase {
    ...
    if (existingWorkflow) {
      await this.sendWebhookMessage.execute({
        ...
        payload: {
          ...
          previousObject: existingWorkflow as unknown as Record<
            string,
            unknown
          >,
        },
  ...
  @Instrument()
  private async buildCreateWorkflowCommand(
    command: UpsertWorkflowCommand
  ): Promise<CreateWorkflowCommandV0> {
    ...
    return {
      ...
      defaultPreferences:
        workflowDto.preferences?.workflow ?? DEFAULT_WORKFLOW_PREFERENCES,
      triggerIdentifier: preserveWorkflowId
        ? workflowDto.workflowId
        : slugifyOrRandom(workflowDto.name),
  ...
  private async buildSteps(
    ...
    for (const step of command.workflowDto.steps) {
      const existingStep: NotificationStepEntity | null | undefined =
        "_id" in step
          ? existingWorkflow?.steps.find(
              (s) => !!step._id && s._templateId === step._id
            )
          : null
      ...
      const generatedStepId =
        ...
        this.generateUniqueStepId(
          step,
          existingWorkflow ? existingWorkflow.steps : tempSteps
        )
    ...
    const optimisticPayloadSchema = command.workflowDto.payloadSchema as
      | JSONSchemaDto
      | undefined
    ...
    const stepsWithIssues = await Promise.all(
      command.workflowDto.steps.map(async (step, index) => {
        ...
        const controlSchemas: ControlSchemas =
          existingStep?.template?.controls ||
          stepTypeToControlSchema[controlSchemaKey]
  ...
  @Instrument()
  private generateUniqueStepId(
    step: UpsertStepDataCommand,
    previousSteps: NotificationStep[]
  ): string {
    ...
    const previousStepIds = previousSteps.reduce<string[]>(
      (acc, { stepId }) => {
        if (stepId) {
          acc.push(stepId)
        }
  ...
  private async upsertControlValues(
    ...
    const controlValuesUpdates = this.getControlValuesUpdates(
    ...
    await Promise.all(
      controlValuesUpdates.map((update) =>
        this.executeControlValuesUpdate(update, updatedWorkflow._id, command)
      )
    )
  }

  @Instrument()
  private getControlValuesUpdates(
    updatedSteps: NotificationStepEntity[],
    command: UpsertWorkflowCommand
  ) {
    return updatedSteps
      .map((step) => {
        const controlValues = this.findControlValueInRequest(
          step,
          command.workflowDto.steps
        )
  ...
  private async executeControlValuesUpdate(
    ...
    if (
      ...
      const shouldApplyStandardEmailProcessing = !isStepResolverActive(
        step.template?.stepResolverHash
      )

      if (
        shouldApplyStandardEmailProcessing &&
        typeof emailControlValues.layoutId === "string"
      ) {
      ...
      if (shouldApplyStandardEmailProcessing) {
        ...
        if (emailControlValues.editorType === "html" && isMaily) {
          ...
          let htmlBody = removeBrandingFromHtml(
            (result.preview as EmailRenderOutput).body ?? ""
          )
          try {
