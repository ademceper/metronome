...
export class CreateNotificationJobs {
    ...
    try {
      ...
      if (isTracesWriteEnabled) {
        const workflowRunIdentifier =
          command.template.triggers?.[0]?.identifier ||
          command.template.name.toLowerCase().replace(/\s+/g, "_")

        const baseTraceData: Omit<
          WorkflowRunTraceInput,
          "event_type" | "title"
        > = {
          ...
          topics: notification.topics
            ? JSON.stringify(notification.topics)
            : "",
  ...
  private buildStepMetadata(
    ...
    if (this.isIDigestBaseMetadata(step.metadata)) {
      const digestValue =
        this.buildDigestValue(step.metadata, command) || "No-Value-Provided"
  ...
  private isIDigestBaseMetadata(obj: unknown): obj is IDigestBaseMetadata {
    ...
    return (
      (typedObj.digestKey === undefined ||
        typeof typedObj.digestKey === "string") &&
      (typedObj.digestValue === undefined ||
        typeof typedObj.digestValue === "string")
    )
  }

  private buildDigestValue(
    metadata: IDigestBaseMetadata,
    command: CreateNotificationJobsCommand
  ): string | undefined {
