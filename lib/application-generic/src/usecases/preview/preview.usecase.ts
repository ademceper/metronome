...
import {
  ...
  shouldIncludeBody,
} from "../../services/http-client/http-request.utils"
...
export class PreviewUsecase {
  ...
  async execute(command: PreviewCommand): Promise<GeneratePreviewResponseDto> {
    try {
      ...
      const { previewTemplateData } =
        this.controlValueSanitizer.processControlValues(
          sanitizedControls,
      ...
      payloadExample =
        this.payloadProcessor.enhanceEventCountValue(payloadExample)

      const cleanedPayloadExample =
        this.payloadProcessor.cleanPreviewExamplePayload(payloadExample)
      ...
      try {
        ...
        const novuSignature = isHttpRequestStep
          ? await this.buildNovuSignatureSample(
  ...
  private applyUrlSafePreviewValues({
    ...
    const redirectVariablePaths = this.getInAppRedirectVariablePaths(
      controlValues,
      variableSchema
    )
    ...
    for (const variablePath of redirectVariablePaths) {
      ...
      const urlSafeValue = this.toUrlSafePreviewValue(
        currentValue,
        variablePath
      )
  ...
  private getInAppRedirectVariablePaths(
    ...
    return [...new Set(variablePaths)].filter((variablePath) =>
      variablePath.startsWith("payload.")
    )
  }
  ...
  private async buildNovuSignatureSample(
    ...
    try {
      ...
      const body = resolvedOutputs?.body as
        | string
