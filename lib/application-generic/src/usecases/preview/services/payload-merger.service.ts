...
export class PayloadMergerService {
  ...
  private async mergeWithPayloadSchema({
    ...
    try {
      ...
      const mockData = JsonSchemaMock.generate(schema) as Record<
        string,
        unknown
      >
    ...
    const fullSubscriberSchema =
      this.mockDataGenerator.createFullSubscriberObject()
    // Preserve user-provided subscriber data even if it was filtered out earlier
    const userSubscriberData =
      (userPayloadExample?.subscriber as Record<string, unknown>) || {}

    mergedPayload.subscriber = merge(
      {},
      fullSubscriberSchema,
      userSubscriberData
    )
    ...
    if (workflow && stepIdOrInternalId) {
       ...
       */
      const stepsFromPayloadExample =
        (payloadExample.steps as Record<string, unknown>) || {}
  ...
  private async createFullStepsObject({
    ...
    const currentStepIndex = workflow.steps.findIndex(
      (step) =>
        step._id === currentStepId || step.stepId === currentStepData.stepId
    )
    ...
    const httpControlValuesMap = await this.getHttpControlValuesMap(
      previousSteps,
      workflow
    )
    ...
    for (const step of previousSteps) {
      ...
      if (stepId) {
        if (userStepsData[stepId]) {
          ...
          const stepControls = step._id
            ? httpControlValuesMap[step._id]
            : undefined
          const responseBodySchema =
            step.template?.type === StepTypeEnum.HTTP_REQUEST
              ? (stepControls?.responseBodySchema as
  ...
  private async getHttpControlValuesMap(
    ...
    const httpRequestStepIds = previousSteps
      .filter(
        (step) => step.template?.type === StepTypeEnum.HTTP_REQUEST && step._id
      )
    ...
    if (httpRequestStepIds.length > 0) {
      ...
      for (const cv of controlValues) {
        if (cv._stepId && httpRequestStepIds.includes(cv._stepId)) {
          httpControlValuesMap[cv._stepId] = cv.controls as Record<
            string,
            unknown
          >
        }
      ...
      if (isPlainObject(value) && isPlainObject(schemaPayload[key])) {
        ...
        filtered[key] = value.map((item) => {
          if (
            isPlainObject(item) &&
            schemaPayload[key] &&
            Array.isArray(schemaPayload[key])
          ) {
            ...
            return this.filterPayloadBySchema(
