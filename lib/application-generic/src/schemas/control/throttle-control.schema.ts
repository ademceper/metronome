...
export const throttleControlZodSchema = z
  .object({
    skip: skipZodSchema,
    type: z
      .enum([ThrottleTypeEnum.FIXED, ThrottleTypeEnum.DYNAMIC])
      .default(ThrottleTypeEnum.FIXED),
  ...
  .refine(
    ...
    {
      message:
        "Fixed throttle requires 'amount' and 'unit', dynamic throttle requires 'dynamicKey'",
    }
  )
