...
export function startOtel(
  ...
  const prometheusPort = parseInt(
    process.env.OTEL_PROMETHEUS_PORT ?? "9464",
    10
  )
  ...
  const metricExportTimeoutMs = parseInt(
  ...
  const logRecordProcessors =
    process.env.ENABLE_OTEL_LOGS === "true"
        ...
        "@opentelemetry/instrumentation-ioredis": {
          ...
          ...(captureDbStatements && {
            dbStatementSerializer: (cmdName, cmdArgs) => {
              const args = (cmdArgs as Array<string | Buffer | number>)
                .map(String)
                .join(" ")
