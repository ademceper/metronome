...
export class ComputeJobWaitDurationService {
  calculateDelay({
    ...
    if (digestType === DelayTypeEnum.SCHEDULED) {
      ...
      if (!dynamicKey)
        throw new BadRequestException(`Dynamic delay key not found`)

      const value = getNestedValue({ payload }, dynamicKey)

      if (!value) {
        throw new BadRequestException(
          `Dynamic delay key '${dynamicKey}' not found in payload`
        )
      }
      ...
      if (typeof value === "string" && DurationUtils.isISO8601(value)) {
        ...
        if (delay < 0) {
          throw new BadRequestException(
            `Dynamic delay timestamp '${value}' must be a future date`
          )
        }
      ...
      if (
        ...
        if (typeof durationObj.amount !== "number" || durationObj.amount < 0) {
          throw new BadRequestException(
            `Invalid amount '${durationObj.amount}' in dynamic delay`
          )
        }

        try {
          return DurationUtils.convertToMilliseconds(
            durationObj.amount,
            durationObj.unit
          )
        } catch {
          throw new BadRequestException(
            `Invalid time unit '${durationObj.unit}' in dynamic delay`
          )
        }
      }
