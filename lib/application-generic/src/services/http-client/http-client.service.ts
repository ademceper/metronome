...
export class HttpClientService {
  ...
  async request<T>(options: HttpRequestOptions): Promise<HttpResponse<T>> {
    ...
    const retryOptions = {
      ...
      calculateDelay: ({
        ...
        if (
          ...
          retryStatusCodes.includes(error.response.statusCode)
        ) {
    ...
    try {
      if (options.enforceSsrfProtection) {
        ...
        return await this.requestSafe<T>({
          url,
          method,
          headers,
          body,
      ...
      if (responseType === "text") {
        return await this.requestText<T>({
          url,
          method,
          headers,
          timeout,
  ...
  private async requestSafe<T>(params: {
    ...
    if (response.statusCode >= 400) {
      throw new HttpClientError({
        ...
        message:
          `Response code ${response.statusCode} (${response.statusMessage || ""})`.trim(),
  ...
  private async requestText<T>(
    params: GotRequestParams
  ): Promise<HttpResponse<T>> {
    const { url, method, headers, timeout, body, retryOptions, httpsOptions } =
      params
