...
describe("http-request.utils", () => {
  ...
  describe("toHeadersRecord", () => {
    it("should convert key-value pairs to a record", () => {
      ...
      expect(toHeadersRecord(pairs)).to.deep.equal({
  ...
  describe("parseRawBody", () => {
    it("should parse valid JSON object", () => {
      ...
      expect(parseRawBody(raw)).to.deep.equal({
        name: "test",
        nested: { key: "value" },
      })
    })
    ...
    it("should support nested objects and arrays", () => {
      const raw =
        '{"voice":{"language":"fr","gender":"Male","number":1},"tags":["a","b"]}'
    ...
    it("should throw for JSON string", () => {
      expect(() => parseRawBody('"hello"')).to.throw(
        "Raw body must be a JSON object or array"
      )
    })

    it("should throw for JSON null", () => {
      expect(() => parseRawBody("null")).to.throw(
        "Raw body must be a JSON object or array"
      )
    })
  })
  ...
  describe("resolveHttpRequestBody", () => {
    it("should parse canonical raw JSON string bodies", () => {
      expect(resolveHttpRequestBody('{"name":"test"}')).to.deep.equal({
        name: "test",
      })
    })
    ...
    it("should convert legacy key-value array bodies", () => {
      expect(
        resolveHttpRequestBody([{ key: "name", value: "test" }])
