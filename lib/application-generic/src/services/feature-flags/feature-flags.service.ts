...
export class FeatureFlagsService {
  ...
  public async initialize(): Promise<void> {
    const Service = process.env.LAUNCH_DARKLY_SDK_KEY
      ? LaunchDarklyFeatureFlagsService
      : ProcessEnvFeatureFlagsService
    ...
    try {
      ...
      Logger.log(
        `Feature Flags service (${Service.name}) has been successfully initialized.`,
        LOG_CONTEXT
      )
    } catch (error) {
      Logger.error(
  ...
  public async getFlag<T_Result>(
    context: FeatureFlagContext<T_Result>
  ): Promise<T_Result> {
