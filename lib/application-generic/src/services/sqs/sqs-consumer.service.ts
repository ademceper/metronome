...
function isNonRetryableDeleteError(error: unknown): boolean {
  ...
  return (
    (typeof errorName === "string" &&
      NON_RETRYABLE_DELETE_ERROR_NAMES.has(errorName)) ||
    (typeof errorCode === "string" &&
      NON_RETRYABLE_DELETE_ERROR_NAMES.has(errorCode))
  )
}
...
export class SqsConsumerService {
  ...
  constructor(
    ...
    const waitTime =
      this.options.waitTimeSeconds ?? SQS_DEFAULT_WAIT_TIME_SECONDS
    const visibilityTimeout =
      this.options.visibilityTimeout ?? SQS_DEFAULT_VISIBILITY_TIMEOUT
    const maxConcurrency =
      this.options.maxConcurrency ?? SQS_DEFAULT_MAX_CONCURRENCY
    ...
    Logger.log(
      { topic: this.topic, batchSize, maxConcurrency },
      ...
      try {
        ...
        const errorMessage =
          deleteError instanceof Error
            ...
            : String(deleteError)
        const errorName =
          deleteError instanceof Error ? deleteError.name : undefined
        ...
        const jitterMs = Math.floor(
          Math.random() * baseBackoffMs * DELETE_BACKOFF_JITTER_FACTOR
        )
  ...
  private setupEventHandlers(): void {
    this.consumer.on("error", (err) => {
      Logger.error(
        { error: err.message, topic: this.topic },
        "SQS consumer error",
        LOG_CONTEXT
      )
    })
  ...
  public async resume(): Promise<void> {
    if (!this.isPaused) {
      Logger.warn(
        { topic: this.topic },
        "Cannot resume SQS consumer: not in paused state",
        LOG_CONTEXT
      )
  ...
  public async stop(options?: { drainTimeoutMs?: number }): Promise<void> {
    ...
    if (drained) {
      Logger.log(
        { topic: this.topic },
        "SQS consumer fully drained and stopped",
        LOG_CONTEXT
      )
    } else {
      Logger.warn(
