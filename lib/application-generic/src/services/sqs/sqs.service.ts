...
export class SqsService {
  ...
  constructor() {
    ...
    const hasConfiguredQueues = Array.from(this.queueUrls.values()).some(
      (url) => url && url.trim() !== ""
    )
    ...
    if (hasConfiguredQueues) {
      ...
      Logger.log(
        {
          message: "SQS service initialized",
          configuredTopics: Array.from(this.producers.keys()),
        },
        LOG_CONTEXT
      )
    } else {
      ...
      Logger.log(
        "SQS service initialized with no queues configured",
        LOG_CONTEXT
      )
    }
  ...
  private initializeClient(): void {
    const region =
      process.env.AWS_REGION || process.env.NOVU_REGION || "us-east-1"
  ...
  private loadQueueUrls(): void {
    this.queueUrls = new Map([
      ...
      [
        ...
        process.env.SQS_QUEUE_URL_PROCESS_SUBSCRIBER,
      ],
  ...
  public async send(
    ...
    const offloadedBody = await this.maybeOffloadBody(
      message.body,
      topic,
      message.id,
      message.groupId
  ...
  public async sendBulk(
    ...
    Logger.debug(
      { message: "Sent bulk messages to SQS", topic, count: messages.length },
      LOG_CONTEXT
    )
  }

  private async maybeOffloadBody(
    body: string,
    topic: JobTopicNameEnum,
    id: string,
    groupId: string
