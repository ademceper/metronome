...
export class SqsPayloadOffloadService {
  ...
  async maybeResolve(body: string): Promise<string> {
    ...
    const { bucket, key } = (parsed as ISqsLargePayloadReference)[
      SQS_LARGE_PAYLOAD_MARKER
    ]
    ...
    Logger.debug(
      { bucket, key },
      "Resolved large SQS payload from S3",
      LOG_CONTEXT
    )
  ...
  private isLargePayloadReference(
    data: unknown
  ): data is ISqsLargePayloadReference {
