...
export type WorkflowRunStatusEventType = Extract<
  EventType,
  `workflow_run_status_${string}`
>
...
type JobResult = Pick<
  JobEntity,
  "type" | "status" | "deliveryLifecycleState" | "_id" | "_mergedDigestId"
>
...
export class WorkflowRunService {
  ...
  async updateDeliveryLifecycle(
    params: WorkflowStatusUpdateParams
  ): Promise<void> {
  ...
  private async updateDeliveryLifecycleWithTransition({
    ...
    try {
      ...
      const [jobs, messages] = await Promise.all([
        this.getJobsForWorkflowRun(
          notificationId,
          environmentId,
          organizationId,
          _subscriberId
        ),
        this.getMessagesForWorkflowRun(
          notificationId,
          environmentId,
          organizationId,
          _subscriberId
      ...
      if (providedStatus) {
        ...
        const result = this.buildDeliveryLifecycle(
          notificationId,
          jobs,
          messages
        )
      ...
      const { emittedStatuses, isUpdated } =
        await this.transitionDeliveryLifecycle({
          notificationId,
          organizationId,
          environmentId,
      ...
      if (isUpdated) {
        ...
        for (const emittedStatus of emittedStatuses) {
          await this.createDeliveryLifecycleTrace(
            ...
            emittedStatus === deliveryLifecycleStatus
              ? deliveryLifecycleDetail
              : undefined,
            notification,
            workflow
    ...
    try {
      ...
      const traceData: WorkflowRunTraceInput = {
        ...
        workflow_run_identifier:
          workflow.triggers?.[0]?.identifier ||
        ...
        payload: notification.payload
          ...
          : "",
        control_values: notification.controls
          ? JSON.stringify(notification.controls)
          : "",
  ...
  private async getNotificationAndWorkflow(
    ...
    passedNotification?: NotificationForTrace | null,
    passedWorkflow?:
      | Pick<NotificationTemplateEntity, "name" | "triggers">
      | WorkflowForTrace
      | null
  ): Promise<{
    notification: NotificationForTrace | null
  ...
  private buildDeliveryLifecycle(
    ...
    const channelJobs = jobs.filter(
      (job) =>
        job.type &&
        ["in_app", "email", "sms", "chat", "push"].includes(job.type)
    )
      ...
      this.logger.info(
        {
          ...
          channelJobStatuses: channelJobs.map((j) => ({
            id: j._id,
            status: j.status,
          })),
    ...
    if (channelJobs.length === 0) {
      ...
      return {
        ...
        deliveryLifecycleDetail:
          DeliveryLifecycleDetail.WORKFLOW_MISSING_CHANNEL_STEP,
      }
    }
    ...
    const allStepsFinished = channelJobs.every((job) =>
    ...
    if (allStepsFinished && skippedJobs.length > 0) {
      ...
      for (const detail of priorityOrder) {
        const jobWithDetail = skippedJobs.find(
          (job) => job.deliveryLifecycleState?.detail === detail
        )
        if (jobWithDetail) {
      ...
      logResolution(DeliveryLifecycleStatusEnum.SKIPPED, {
    ...
    const allStepsFailed = channelJobs.every(
      (job) => job.status === JobStatusEnum.FAILED
    )
  ...
  private shouldCreateTrace(
    ...
    switch (deliveryLifecycleStatus) {
      case DeliveryLifecycleStatusEnum.SENT: {
        const completedWithMessage = channelJobs.filter(
          (job) =>
            ...
            messages.some((m) => m._jobId === job._id)
        )
        ...
        const isCurrentJobInCompleted = completedWithMessage.some(
      ...
      case DeliveryLifecycleStatusEnum.DELIVERED: {
        ...
        if (currentJob) {
          const isCurrentJobDelivered = deliveredMessages.some(
            (m) => m._jobId === currentJob._id
          )
      ...
      case DeliveryLifecycleStatusEnum.INTERACTED: {
        const interactedMessages = messages.filter(
          (m) => m.seen || m.read || m.snoozedUntil || m.archived
        )
  ...
  private async transitionDeliveryLifecycle(params: {
    ...
    emittedStatuses: DeliveryLifecycleStatusEnum[]
    isUpdated: boolean
  }> {
    ...
    if (
      params.isInAppChannel &&
      params.targetStatus === DeliveryLifecycleStatusEnum.DELIVERED
    ) {
  ...
  private async tryNotificationDeliveryLifecycleTransition(params: {
    ...
    isUpdated: boolean
    previousEvent?: DeliveryLifecycleEventType
  }> {
    try {
...
function isJobCancelled(job: JobResult): boolean {
  return (
    job.status === JobStatusEnum.CANCELED && !job.deliveryLifecycleState?.status
  )
}
