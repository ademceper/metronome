      ...
      ;[JsonLogicOperatorEnum.AND, JsonLogicOperatorEnum.OR].forEach(
        (operator) => {
          ...
          it(`should detect invalid ${operator} structure`, () => {
            ...
            expect(issues).to.have.lengthOf(1)
            expect(issues[0].message).to.include(
              `Invalid logical operator "${operator}"`
            )
            expect(issues[0].path).to.deep.equal([])
            expect(issues[0].type).to.equal(
              QueryIssueTypeEnum.INVALID_STRUCTURE
            )
          })
        }
      )
    ...
    describe("in operation", () => {
      ...
      describe('"in" operation', () => {
        ...
        it('should detect invalid field reference in "in" operation', () => {
          ...
          expect(issues).to.have.lengthOf(1)
          expect(issues[0].message).to.include(
            "Invalid field reference in comparison"
          )
          expect(issues[0].path).to.deep.equal([])
          expect(issues[0].type).to.equal(QueryIssueTypeEnum.INVALID_STRUCTURE)
        })
