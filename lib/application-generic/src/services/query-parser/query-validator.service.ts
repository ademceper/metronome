...
export class QueryValidatorService {
  ...
  private isInvalidFieldValue(field: unknown) {
    ...
    const isWithinAllowedPrefixes = this.allowedNamespaces.some(
      (prefix) =>
        fieldValue.startsWith(prefix) && fieldValue.length > prefix.length
    )

    const normalizedFieldValue = this.normalizeArrayNotation(fieldValue)
    const normalizedAllowedVariables = this.allowedVariables.map((v) =>
      this.normalizeArrayNotation(v)
    )
    const isInAllowedVariables =
      normalizedAllowedVariables.includes(normalizedFieldValue)

    return !fieldValue || (!isInAllowedVariables && !isWithinAllowedPrefixes)
  }

  private getLogicalOperatorIssue(
    operator: string,
    path: number[]
  ): QueryIssue {
    return {
  ...
  private validateNode({
    ...
    for (const [key, value] of entries) {
      ...
      if (
        [JsonLogicOperatorEnum.AND, JsonLogicOperatorEnum.OR].includes(
          key as JsonLogicOperatorEnum
        )
      ) {
      ...
      if (key === JsonLogicOperatorEnum.NOT) {
        this.validateNode({
          node: value as RulesLogic<AdditionalOperation>,
          issues,
          path,
        })
        continue
      ...
      if (key === "containsAny" || key === "doesNotContainAny") {
        this.validateContainsAnyOperation({
          operator: key,
          value,
          issues,
          path,
      ...
      const isBetween =
        key === JsonComparisonOperatorEnum.LESS_THAN_OR_EQUAL &&
        Array.isArray(value) &&
        value.length === 3
      if (isBetween) {
  ...
  private validateBetweenOperation({
    ...
    const lowerBoundIsUndefined =
      lowerBound === undefined || lowerBound === null
    const upperBoundIsUndefined =
  ...
  private validateComparisonOperation({
    ...
    const invalidComparisonValue =
      operator === "in" && !Array.isArray(comparisonValue)
  ...
  private validateInOperation({
    ...
    if (isContains) {
      ...
      const searchValueExists =
        firstOperand === undefined || firstOperand === ""
      if (searchValueExists) {
        issues.push(this.getValueIssue(path))
      }

      // Validate field reference
      const secondOperandInvalid =
        !secondOperand ||
        typeof secondOperand !== "object" ||
        !("var" in secondOperand)
      if (secondOperandInvalid) {
        issues.push(this.getFieldReferenceIssue(path))
      }
    } else {
      // Validate field reference
      const firstOperandInvalid =
        !firstOperand ||
