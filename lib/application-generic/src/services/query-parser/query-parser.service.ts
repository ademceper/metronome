...
function createStringOperator(
  evaluator: (input: string, value: string) => boolean
) {
...
const initializeCustomOperators = (): void => {
  ...
  jsonLogic.add_operation(
    "null",
    (dataInput: unknown): boolean => dataInput === null
  )
  ...
  jsonLogic.add_operation(
    "notIn",
    (dataInput: unknown, ruleValue: unknown[]): boolean =>
      Array.isArray(ruleValue) && !ruleValue.includes(dataInput)
  )
  ...
  jsonLogic.add_operation(
    ...
    (dataInput: unknown, ruleValue: unknown): boolean => {
      ...
      const targetDate = calculateRelativeDate(
        validation.amount,
        validation.unit
      )
