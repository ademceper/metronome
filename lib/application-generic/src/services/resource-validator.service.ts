...
export class ResourceValidatorService {
  ...
  async validateWorkflowLimit(environmentId: string): Promise<void> {
    ...
    const maxWorkflowLimit = await this.getWorkflowLimit(
      environment,
      organization
    )
  ...
  private async getWorkflowLimit(
    environment: EnvironmentEntity,
    organization: OrganizationEntity
  ) {
    const systemLimitMaxWorkflow = await this.getMaxWorkflowSystemLimit(
      environment,
      organization
    )
    ...
    const maxWorkflowsTierLimit = await this.getMaxWorkflowsTierLimit(
      environment,
      organization
    )
  ...
  async validateStepResolversLimit(
    ...
    const environment = await this.getEnvironment(environmentId)
    const organization = await this.getOrganization(organizationId)
    const maxStepResolversLimit = await this.getStepResolversLimit(
      environment,
      organization
    )
  ...
  async getStepResolversAvailableSlots(
    environmentId: string,
    organizationId: string
  ): Promise<number> {
    if (process.env.IS_SELF_HOSTED === "true") {
  ...
  private async getStepResolversLimit(
    environment: EnvironmentEntity,
    organization: OrganizationEntity
  ) {
    const systemLimitMaxStepResolvers =
      await this.getMaxStepResolversSystemLimit(environment, organization)
    const isSpecialLimit =
      systemLimitMaxStepResolvers !== SYSTEM_LIMITS.STEP_RESOLVERS

    if (isSpecialLimit) {
      return systemLimitMaxStepResolvers
    }

    const maxStepResolversTierLimit =
      await this.getMaxStepResolversTierLimit(organization)

    return Math.min(systemLimitMaxStepResolvers, maxStepResolversTierLimit)
  }

  private async getMaxStepResolversSystemLimit(
    environment: EnvironmentEntity,
    organization: OrganizationEntity
  ) {
    return await this.featureFlagService.getFlag({
  ...
  async validateLayoutsLimit(
    ...
    const environment = await this.getEnvironment(environmentId)
    const organization = await this.getOrganization(environment._organizationId)
    const maxLayoutsLimit = await this.getLayoutLimit(
      environment,
      organization,
      layoutsCount
    )
    ...
    if (layoutsCount >= maxLayoutsLimit) {
      throw new BadRequestException({
        message:
          "Layout limit exceeded. Please contact us to support more layouts.",
  ...
  private async getLayoutLimit(
    ...
    if (
      layoutsCount >= maxLayoutsTierLimit &&
      organization.apiServiceLevel === ApiServiceLevelEnum.FREE
    ) {
      return maxLayoutsTierLimit
    }

    const systemLimitMaxLayouts = await this.getMaxLayoutsSystemLimit(
      environment,
      organization
    )
    // If the system limit is not the default, we need to use it as the absolute limit for special cases instead of the tier limit
  ...
  async validateEnvironmentVariablesLimit(
    organizationId: string
  ): Promise<void> {
    const variablesCount = await this.environmentVariableRepository.count({
