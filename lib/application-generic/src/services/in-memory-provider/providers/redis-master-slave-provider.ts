...
export const getRedisMasterSlaveProviderConfig =
  (): IRedisMasterSlaveProviderConfig => {
    ...
    const masterPort = redisMasterSlaveConfig.masterPort
      ? Number(redisMasterSlaveConfig.masterPort)
      : 6379
    ...
    const family = redisMasterSlaveConfig.family
      ? Number(redisMasterSlaveConfig.family)
      : DEFAULT_FAMILY
    const keepAlive = redisMasterSlaveConfig.keepAlive
      ? Number(redisMasterSlaveConfig.keepAlive)
      : DEFAULT_KEEP_ALIVE
    const keyPrefix = redisMasterSlaveConfig.keyPrefix ?? DEFAULT_KEY_PREFIX
    const ttl = redisMasterSlaveConfig.ttl
      ? Number(redisMasterSlaveConfig.ttl)
      : DEFAULT_TTL_SECONDS
...
export const getRedisMasterSlaveCluster = (
