...
export class WebSocketsQueueService extends QueueBaseService {
  ...
  public async add(data: IWebSocketJobDto) {
    ...
    if (isSocketWorkerEnabled && data.data) {
      ...
      Logger.debug(
        { userId, event },
        "Sent message directly to socket worker",
        LOG_CONTEXT
      )

      const isLegacyWsDisabled =
        await this.socketWorkerService.isLegacyWsDisabled(
          data.data._environmentId,
      ...
      if (isLegacyWsDisabled) {
        Logger.debug(
          { userId },
          "Legacy WS service is disabled, skipping queue push",
          LOG_CONTEXT
        )
  ...
  public async addBulk(data: IWebSocketBulkJobDto[]): Promise<void> {
    ...
    if (isSocketWorkerEnabled) {
      ...
      if (isLegacyWsDisabled) {
        Logger.debug(
          "Legacy WS service is disabled, skipping bulk queue push",
          LOG_CONTEXT
        )
