  ...
  private async handleCFSchedulerMode(
    ...
    switch (mode) {
      ...
      case "live":
        this.logger.info(
          { jobId: originalData.data._id },
          "Live mode: CF Scheduler will process, BullMQ is shadow"
        )
      ...
      case "complete":
        this.logger.info(
          { jobId: originalData.data._id },
          "Complete mode: Adding only to CF Scheduler"
        )
        await this.cloudflareSchedulerService.scheduleJob(schedulerRequest)
        break
      ...
      default:
        this.logger.warn(
          { mode },
          "Unknown CF Scheduler mode, falling back to BullMQ"
        )
        await super.add(originalData)
    }
  }
