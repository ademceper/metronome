...
export class QueueBaseService implements OnModuleDestroy {
  ...
  public async gracefulShutdown(): Promise<void> {
    ...
    Logger.log(
      { topic: this.topic },
      "Queue service shutdown complete",
      LOG_CONTEXT
    )
  }
  ...
  public async add(params: IJobParams) {
    ...
    if (!organizationId) {
      Logger.debug(
        { topic: this.topic },
        "Job without organization ID, routing to BullMQ fallback",
        LOG_CONTEXT
      )
    ...
    Logger.debug(
      { topic: this.topic, queueBackendMode, organizationId },
  ...
  private async getQueueBackendMode(
    ...
    if (!organization) {
      Logger.warn(
        { organizationId, topic: this.topic },
        "Organization not found, skipping job",
        LOG_CONTEXT
      )
  ...
  private async routeByMode(
    ...
    switch (queueBackendMode) {
      ...
      case QueueBackendMode.SHADOW: {
        ...
        try {
          await this.addJobsToSQS(
            this.markAsSkipProcessing(jobs),
            organizationId
          )
        } catch (error) {
          this.logger?.warn(
            ...
            Logger.warn(
              {
                ...
                count: jobs.length,
                error:
                  bullmqError instanceof Error
                    ? bullmqError.message
                    : String(bullmqError),
                stack:
                  bullmqError instanceof Error ? bullmqError.stack : undefined,
              },
      ...
      default:
        Logger.warn(
          ...
          "Unknown queue backend mode, falling back to BullMQ",
          LOG_CONTEXT
        )
        return await this.addJobsToBullMQ(jobs)
    }
  ...
  private async addJobsToSQS(
    ...
    if (messages.length === 1) {
      ...
      Logger.debug(
        {
          topic: this.topic,
          jobName: jobs[0].name,
          payloadSizeBytes: this.calculatePayloadSize(jobs[0].data),
        },
        "Added job to SQS",
        LOG_CONTEXT
      )
      ...
      Logger.debug(
        { topic: this.topic, count: messages.length },
  ...
  private logBulkPayloadMetrics(data: IBulkJobParams[]): void {
    const payloadSizes = data.map((item) =>
      this.calculatePayloadSize(item.data)
    )
    ...
    const avgPayloadSize =
      validSizes.length > 0
        ? Math.round(totalPayloadSize / validSizes.length)
        : 0
  ...
  private calculatePayloadSize(data: any): number {
    ...
    try {
      ...
      Logger.warn(
        { error: errorMessage },
        "Failed to calculate payload size",
        LOG_CONTEXT
      )
