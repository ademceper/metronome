...
@Injectable()
export class StepRunRepository extends LogRepository<
  typeof stepRunSchema,
  StepRun
> {
  ...
  private mapStepTypeEnumToStepType(
  ...
  async create(job: JobEntity, options: StepOptions = {}): Promise<void> {
    try {
      ...
      this.logger.error(
        { err: error, jobId: job._id, status: job.status },
        `Failed to log step ${job.status}`
      )
    }
  }
  ...
  async getAvgMessagesPerSubscriberData(
    ...
    const currentTotalStepRuns = parseInt(
      currentResult.data[0]?.total_step_runs || "0",
      10
    )
    const currentUniqueSubscribers = parseInt(
      currentResult.data[0]?.unique_subscribers || "0",
      10
    )
    const previousTotalStepRuns = parseInt(
      previousResult.data[0]?.total_step_runs || "0",
      10
    )
    const previousUniqueSubscribers = parseInt(
      previousResult.data[0]?.unique_subscribers || "0",
      10
    )

    // Calculate averages (handle division by zero)
    const currentPeriod =
      currentUniqueSubscribers > 0
        ? currentTotalStepRuns / currentUniqueSubscribers
        : 0
    const previousPeriod =
      previousUniqueSubscribers > 0
        ? previousTotalStepRuns / previousUniqueSubscribers
        : 0
  ...
  private mapJobToStepRun(
    job: JobEntity,
    options?: StepOptions
  ): StepRunInsertData {
    const now = new Date()
    const stepType = this.mapStepTypeEnumToStepType(
      job.type || job.step.template?.type
    )
