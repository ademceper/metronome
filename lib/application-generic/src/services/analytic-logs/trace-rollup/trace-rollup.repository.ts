...
> {
  ...
  async getAvgMessagesPerSubscriberData(
    ...
    const currentTotalMessages = parseInt(
      currentResult.data[0]?.total_messages || "0",
      10
    )
    ...
    const previousTotalMessages = parseInt(
      previousResult.data[0]?.total_messages || "0",
      10
    )
    ...
    const currentPeriod =
      currentUniqueSubscribers > 0
        ? currentTotalMessages / currentUniqueSubscribers
        : 0
    const previousPeriod =
      previousUniqueSubscribers > 0
        ? previousTotalMessages / previousUniqueSubscribers
        : 0
