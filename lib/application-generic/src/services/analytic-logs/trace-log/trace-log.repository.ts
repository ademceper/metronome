...
@Injectable()
export class TraceLogRepository extends LogRepository<
  typeof traceLogSchema,
  Trace
> {
  ...
  private async createMany(
    traceDataArray: Omit<Trace, "id" | "expires_at">[]
  ): Promise<void> {
      ...
      this.logger.debug(
        {
          ...
          entityTypes: [
            ...new Set(traceDataArray.map((trace) => trace.entity_type)),
          ],
          eventTypes: [
            ...new Set(traceDataArray.map((trace) => trace.event_type)),
          ],
        },
        "Trace events logged"
      )
  ...
  async getInteractionTrendData(
    ...
    const workflowFilter =
      workflowIds && workflowIds.length > 0
        ? `AND traces.workflow_id IN {workflowIds:Array(String)}`
        : ""
  ...
  async getUsageReportBreakdown(
    ...
    Array<{ step_run_type: string; provider_id: string; count: string }>
  > {
    if (environmentIds.length === 0) {
      this.logger.info(
