...
export class ClickHouseService implements BeforeApplicationShutdown {
  ...
  async init() {
    ...
    if (process.env.NODE_ENV === "local" || process.env.NODE_ENV === "test") {
      ...
      try {
        ...
        console.error(
          `Failed to create database ${process.env.CLICK_HOUSE_DATABASE}:`,
          error
        )
      }
    }
  ...
  async ping(): Promise<PingResult> {
    if (!this._client) {
      return {
        success: false,
        error: new Error("Ping failed: ClickHouse client not initialized"),
      }
    }
  ...
  public async insert<T extends Record<string, unknown>>(
    ...
    if (clickhouseSettings?.waitForAsyncInsert !== undefined) {
      settings.wait_for_async_insert = clickhouseSettings.waitForAsyncInsert
        ? 1
        : 0
    }
