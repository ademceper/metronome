...
export const ALLOWED_OPERATORS: readonly ClickhouseOperator[] =
...
type ConditionValue<
  T,
  K extends keyof T,
  O extends ClickhouseOperator,
> = O extends NullOperators ? never : O extends ArrayOperators ? T[K][] : T[K]

export type FieldCondition<
  T,
  K extends keyof T,
  O extends ClickhouseOperator,
> = O extends NullOperators
  ? {
      field: K
      operator: O
    }
...
type WhereCondition<T> =
  | FieldCondition<T, keyof T, ClickhouseOperator>
  | OrCondition<T>
...
export type SchemaKeys<T extends ClickhouseSchema<any>> =
  keyof InferClickhouseSchemaType<T>

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export abstract class LogRepository<
  TSchema extends ClickhouseSchema<any>,
  TEnhancedType,
> {
  ...
  protected buildWhereClause(where: Where<TEnhancedType>): {
    ...
    const rawWhere = where as unknown as Where<
      InferClickhouseSchemaType<TSchema>
    >
  ...
  private buildEnforcedConditions(
    enforced: EnforcedContext
  ): WhereCondition<InferClickhouseSchemaType<TSchema>>[] {
    const condition = {
    ...
    const conditions: WhereCondition<InferClickhouseSchemaType<TSchema>>[] = [
      condition,
    ]
  ...
  private buildWhereClauseFromConditions(
    conditions: WhereCondition<InferClickhouseSchemaType<TSchema>>[]
  ): {
    clause: string
    params: Record<string, unknown>
  } {
    ...
    const buildSingleCondition = (
      condition: WhereCondition<InferClickhouseSchemaType<TSchema>>
    ): string => {
      ...
      if ("$or" in condition) {
        ...
        const orClauses = condition.$or.map((orCondition) =>
          buildSingleCondition(orCondition)
        )
      ...
      if (!("field" in condition) || !("operator" in condition)) {
        throw new Error(
          "Each condition must have field and operator properties"
        )
      }
      ...
      if (
        !nullOperators.includes(operator as NullOperators) &&
        (value === null || value === undefined)
      ) {
        throw new Error(
          `Invalid value for column '${String(field)}': value cannot be null or undefined`
        )
      }
      ...
      if (
        arrayOperators.includes(operator as ArrayOperators) &&
        Array.isArray(value)
      ) {
  ...
  protected async insert(
    ...
    const id: string =
      data?.id || `${this.identifierPrefix}${generateObjectId()}`
  ...
  protected getBatchConfig(): {
    ...
    const parsedMaxBatchSize = maxBatchSizeEnv
      ? parseInt(maxBatchSizeEnv, 10)
      : defaultMaxBatchSize
    const maxBatchSize =
      Number.isFinite(parsedMaxBatchSize) && parsedMaxBatchSize > 0
        ? parsedMaxBatchSize
        : defaultMaxBatchSize

    const flushIntervalMsEnv = process.env[`${tableName}_FLUSH_INTERVAL_MS`]
    const parsedFlushIntervalMs = flushIntervalMsEnv
      ? parseInt(flushIntervalMsEnv, 10)
  ...
  protected async insertMany(
    ...
    const ids = data.map(
      (_item) => `${this.identifierPrefix}${generateObjectId()}`
    )
    const expirationDate = await this.getExpirationDate(context)
    const expiresAt = LogRepository.formatDateTime64(expirationDate)
  ...
  async find<
    T extends readonly (keyof InferClickhouseSchemaType<TSchema>)[],
  >(options: {
  ...
  >(options: {
    ...
    data:
      ...
      | Prettify<
          Pick<
            TEnhancedType,
            T extends readonly (keyof TEnhancedType)[] ? T[number] : never
          >
        >[]
    rows: number
  }> {
    ...
    if (orderDirection && !ORDER_DIRECTION.includes(orderDirection)) {
      throw new Error(
        `Invalid order direction: ${orderDirection}. Allowed directions: ${ORDER_DIRECTION.join(", ")}`
      )
    }

    // Build SELECT clause - use provided columns or all columns if "*" is specified
    const selectClause =
      select === "*" ? "*" : (select as readonly string[]).join(", ")
  ...
  >(options: {
    ...
    const result = await this.find({
      ...
      limit: 1,
      select:
        options.select as T extends readonly (keyof InferClickhouseSchemaType<TSchema>)[]
          ? T
          : never,
    } as Parameters<typeof this.find>[0])
...
export class QueryBuilder<T> {
   ...
   * ```
   */
  whereHas<K extends keyof T>(
    field: K,
    value: T[K] extends readonly (infer U)[] ? U : T[K]
  ): this {
   ...
   */
  orWhere(
    orConditions: Array<FieldCondition<T, keyof T, ClickhouseOperator>>
  ): this {
