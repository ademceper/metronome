 ...
 */
@Injectable()
export class ClickHouseBatchService
  implements OnModuleDestroy, OnModuleInit, BeforeApplicationShutdown
{
  ...
  async add<T extends Record<string, unknown>>(
    ...
    if (!this.clickhouseService.client) {
      this.logger.debug(
        { table },
        "ClickHouse client not initialized, skipping batch add"
      )
    ...
    const backpressureMode =
      config.backpressureMode ?? DEFAULT_BACKPRESSURE_MODE
    ...
    if (buffer.rows.length >= maxQueueDepth) {
      ...
      const result = await Promise.race([
        ...
        this.sleep(DEFAULT_BACKPRESSURE_TIMEOUT_MS).then(
          () => "timeout" as const
        ),
      ])
    ...
    if (buffer.rows.length >= config.maxBatchSize) {
      this.logger.debug(
        { table, bufferSize: buffer.rows.length },
        "Max batch size reached, triggering flush"
      )
  ...
  private async flushBatchWithRetry(
    ...
    try {
      ...
      if (retryCount < maxRetries) {
        ...
        return this.flushBatchWithRetry(
          table,
          batch,
          insertOptions,
          maxRetries,
  ...
  private async getTotalActiveJobsCount(): Promise<number> {
    const counts = await Promise.all(
      this.queueServices.map((queue) => queue.getActiveCount())
    )
  ...
  private async waitForActiveJobsToComplete(): Promise<void> {
    if (this.queueServices.length === 0) {
      this.logger.debug(
        "No queue services configured, skipping active jobs wait"
      )
    ...
    for (let attempt = 1; attempt <= SHUTDOWN_MAX_ATTEMPTS; attempt++) {
      ...
      if (totalActiveJobs === 0) {
        this.logger.info(
          "All active jobs completed, proceeding with final flush"
        )
      ...
      this.logger.info(
        {
          ...
          attempt,
          maxAttempts: SHUTDOWN_MAX_ATTEMPTS,
        },
