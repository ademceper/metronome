...
type WorkflowRunColumns = keyof InferClickhouseSchemaType<
  typeof workflowRunSchema
>

// Utility type to create partial WorkflowRun based on selected columns
type SelectedWorkflowRun<T extends readonly WorkflowRunColumns[]> = Pick<
  WorkflowRun,
  T[number]
>
...
@Injectable()
export class WorkflowRunRepository extends LogRepository<
  typeof workflowRunSchema,
  WorkflowRun
> {
  public readonly table = TABLE_NAME
  public readonly schema = workflowRunSchema
  public readonly schemaOrderBy: SchemaKeys<typeof workflowRunSchema>[] =
    ORDER_BY
  ...
  async create(
    ...
    try {
      ...
      const workflowRunData = this.mapNotificationToWorkflowRun(
        notification,
        workflow,
        options
      )
  ...
  async updateWorkflowRunState(
    ...
    try {
      ...
      const workflowRunData = this.mapNotificationToWorkflowRun(
        notification,
        workflow as NotificationTemplateEntity,
        {
          status,
  ...
  async findWithCursor<T extends readonly WorkflowRunColumns[] | "*">(options: {
    ...
    data:
      | WorkflowRun[]
      | SelectedWorkflowRun<
          T extends readonly WorkflowRunColumns[] ? T : never
        >[]
    rows: number
  }> {
    const {
    ...
    const { clause: baseClause, params: baseParams } =
    ...
    this.logger.debug(
      {
        ...
        selectedColumns:
          select === "*" ? "all" : (select as readonly string[]).length,
  ...
  private mapNotificationToWorkflowRun(
    ...
    return {
      created_at: LogRepository.formatDateTime64(
      ...
      topics: notification.topics
        ? notification.topics && JSON.stringify(notification.topics)
        : null,
      ...
      // Delivery lifecycle
      ...(options.deliveryLifecycleStatus && {
        delivery_lifecycle_status: options.deliveryLifecycleStatus,
      }),
      ...(options.deliveryLifecycleDetail && {
        delivery_lifecycle_detail: options.deliveryLifecycleDetail,
      }),
  ...
  async getWorkflowVolumeData(
    ...
    const workflowFilter =
      workflowIds && workflowIds.length > 0
        ? "AND workflow_id IN {workflowIds:Array(String)}"
        : ""
