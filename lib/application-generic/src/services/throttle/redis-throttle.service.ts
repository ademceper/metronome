...
export class RedisThrottleService {
  ...
  private buildSetKey(params: {
    ...
    const throttleKeyPart =
      params.throttleKey && params.throttleValue !== undefined
        ? `:${params.throttleKey}:${params.throttleValue}`
        : ""
  ...
  private async ensureScriptsLoaded(): Promise<void> {
    ...
    try {
      if (!this.reserveScriptSha) {
        this.reserveScriptSha = (await client.script(
          "LOAD",
          this.reserveScript
        )) as string
      }
      if (!this.releaseScriptSha) {
        this.releaseScriptSha = (await client.script(
          "LOAD",
          this.releaseScript
        )) as string
      }
  ...
  async reserveThrottleSlot(
    params: IThrottleReservationParams
  ): Promise<IThrottleReservationResult> {
