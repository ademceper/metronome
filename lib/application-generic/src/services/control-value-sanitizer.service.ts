...
export class ControlValueSanitizerService {
  ...
  sanitizeControlsForPreview(
    ...
    const sanitizedValues = dashboardSanitizeControlValues(
      this.logger,
      initialControlValues,
      type
    )
    const sanitizedByOutputSchema = this.sanitizeControlValuesByOutputSchema(
      sanitizedValues || {},
      type
    )
  ...
  processControlValues(
    ...
    for (const [controlKey, controlValue] of Object.entries(
      controlValues || {}
    )) {
      ...
      const controlValueWithFixedVariables =
        this.fixControlValueInvalidVariables(
          controlValue,
          variables.invalidVariables
        )

      const processedControlValues =
        this.sanitizeControlValuesByLiquidCompilationFailure(
          controlKey,
  ...
  private sanitizeControlValuesByOutputSchema(
    ...
    let outputSchema =
      channelStepSchemas[type]?.output || actionStepSchemas[type]?.output
    if (type === "layout") {
  ...
  private fixControlValueInvalidVariables(
    ...
    try {
      ...
      let controlValuesString = isMailyJSONContent
        ? controlValue
      ...
      for (const invalidVariable of invalidVariables) {
        ...
        controlValuesString = replaceAll(
          controlValuesString,
          variableOutput,
          EMPTY_STRING
        )
  ...
  private sanitizeControlValuesByLiquidCompilationFailure(
    key: string,
    value: unknown
  ): unknown {
