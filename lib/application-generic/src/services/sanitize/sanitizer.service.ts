...
export const sanitizeHTML = (html: string): string => {
  ...
  const cleanHtmlWithDocType = doctypeTags
    ? doctypeTags[0] + cleanHtml
    : cleanHtml

  return cleanHtmlWithDocType
}

export const sanitizeHtmlInObject = <T extends Record<string, unknown>>(
  object: T
): T => {
