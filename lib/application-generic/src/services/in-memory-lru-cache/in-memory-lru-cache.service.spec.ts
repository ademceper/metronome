...
describe("InMemoryLRUCacheService", () => {
  ...
  describe("get", () => {
    ...
    it("should deduplicate concurrent requests", async () => {
      ...
      const [result1, result2, result3] = await Promise.all([
        service.get(InMemoryLRUCacheStore.WORKFLOW, "key4", fetchFn, {
          environmentId: "env1",
        }),
        service.get(InMemoryLRUCacheStore.WORKFLOW, "key4", fetchFn, {
          environmentId: "env1",
    ...
    it("should handle different stores independently", async () => {
      ...
      await service.get(InMemoryLRUCacheStore.WORKFLOW, "key7", workflowFn, {
        environmentId: "env1",
      })
      await service.get(InMemoryLRUCacheStore.ORGANIZATION, "key7", orgFn, {
        environmentId: "env1",
      })
    ...
    it("should isolate cache entries by cacheVariant", async () => {
      ...
      const fetchFn1 = jest
        .fn()
        .mockResolvedValue({ id: "1", projection: "variant1" })
      const fetchFn2 = jest
        .fn()
        .mockResolvedValue({ id: "2", projection: "variant2" })
  ...
  describe("getIfCached", () => {
    it("should return undefined for non-existent key", () => {
      const result = service.getIfCached(
        InMemoryLRUCacheStore.WORKFLOW,
        "nonexistent"
      )
    ...
    it("should return cached value without calling fetch", async () => {
      ...
      await service.get(InMemoryLRUCacheStore.WORKFLOW, "key8", fetchFn, {
        environmentId: "env1",
      })
  ...
  describe("invalidate", () => {
    it("should remove specific key from cache", async () => {
      ...
      await service.get(InMemoryLRUCacheStore.WORKFLOW, "key9", fetchFn, {
        environmentId: "env1",
      })
  ...
  describe("invalidateAll", () => {
    it("should clear entire store", async () => {
      ...
      await service.get(InMemoryLRUCacheStore.WORKFLOW, "key10", fetchFn1, {
        environmentId: "env1",
      })
      await service.get(InMemoryLRUCacheStore.WORKFLOW, "key11", fetchFn2, {
        environmentId: "env1",
      })
