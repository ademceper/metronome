...
import {
  type CacheStoreDataTypeMap,
  type InMemoryLRUCacheStore,
  STORE_CONFIGS,
  type StoreConfig,
...
export class InMemoryLRUCacheService {
  ...
  async get<TStore extends InMemoryLRUCacheStore>(
    ...
    const store =
      this.getOrCreateStore<CacheStoreDataTypeMap[TStore]>(storeName)
  ...
  getIfCached<TStore extends InMemoryLRUCacheStore>(
    ...
    const keyValue = store.cache.get(key) as
      | CacheStoreDataTypeMap[TStore]
      | undefined
  ...
  private async isCacheEnabled(
    ...
    try {
      const flagContext = {
        ...
        ...(opts.organizationId && {
          organization: { _id: opts.organizationId },
        }),
      }
