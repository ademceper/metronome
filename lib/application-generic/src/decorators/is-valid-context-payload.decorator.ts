...
function validateContextDataSizes(
  contextObj: Record<string, unknown>
): ContextPayloadValidationResult {
...
function validateSingleContextData(
  contextType: string,
...
// Decorators
export const IsValidContextPayload =
  createValidationDecorator<ContextPayloadValidationOptions>(
    "isValidContextPayload",
    (value, options) =>
      validateContextPayloadWithDetails(value, options?.maxCount),
    "Invalid context payload"
  )

export const IsValidContextData =
  createValidationDecorator<ContextDataValidationOptions>(
    "isValidContextData",
    (value, options) =>
      validateContextDataWithDetails(value, options?.maxSizeKB),
