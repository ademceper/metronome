...
export interface IHandler {
  ...
  autoConfigureInboundWebhook: (configurations: {
    webhookUrl: string
  }) => Promise<{
    success: boolean
    message?: string
...
export abstract class BaseHandler<T extends ChannelProvider = ChannelProvider>
  implements IHandler
{
  ...
  public parseEventBody(
    ...
    return result && typeof result === "object"
      ? (result as IEmailEventBody | ISMSEventBody)
      : undefined
  }
  ...
  public async verifySignature({
    ...
    if (!this.provider?.verifySignature) {
      ...
      return {
        success: true,
        message:
          "A support of signature verification is not implemented by provider",
      }
    }
  ...
  public async autoConfigureInboundWebhook(configurations: {
    webhookUrl: string
  }): Promise<{
    success: boolean
    message?: string
