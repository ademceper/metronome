...
> {
  ...
  async getFeed(
    ...
    if (query.subscriptionId) {
      requestQuery["topics.preferenceEvaluation.subscriptionIdentifier"] =
  ...
  public async findMetadataForTraces(
    ...
    return this.mapEntity(
      await this.populateFeedWithoutExecutionDetails(
        this.MongooseModel.findOne(requestQuery),
        _environmentId
      )
    ) as unknown as NotificationFeedItemEntity
  ...
  public async findNotificationMetadataOnly(
    ...
    return this.mapEntity(
      await this.populateNotificationMetadataOnly(
        this.MongooseModel.findOne(requestQuery)
      )
    ) as unknown as NotificationFeedItemEntity
  }
  ...
  private populateNotificationMetadataOnly(
    query: QueryWithHelpers<unknown, unknown, unknown>
  ) {
    return query
      .populate({
  ...
  async tryDeliveryLifecycleTransition(
    ...
    return {
      ...
      previousEvent: result?.lastEmittedDeliveryEvent as
        | DeliveryLifecycleEventType
        | undefined,
    }
  }
