...
const LIST_CONVERSATIONS_SORT_FIELDS = [
  "_id",
  "createdAt",
  "lastActivityAt",
] as const
type ListConversationsSortField =
  (typeof LIST_CONVERSATIONS_SORT_FIELDS)[number]

function resolveListConversationsSortBy(
  sortBy?: string
): ListConversationsSortField {
  if (
    sortBy &&
    (LIST_CONVERSATIONS_SORT_FIELDS as readonly string[]).includes(sortBy)
  ) {
...
> {
  ...
  async listConversations({
    ...
    const afterCursor =
      after && conversation
        ? {
            sortBy: conversation[validatedSortBy],
            paginateField: conversation._id,
          }
        : undefined
    const beforeCursor =
      before && conversation
    ...
    if (subscriberId) {
      query.participants = {
        $elemMatch: {
          id: subscriberId,
          type: ConversationParticipantTypeEnum.SUBSCRIBER,
        },
      }
    }
    ...
    return this.findWithCursorBasedPagination({
      ...
      limit,
      sortDirection:
        sortDirection === 1 ? DirectionEnum.ASC : DirectionEnum.DESC,
