...
> {
  ...
  > | null> {
    return this.findOne(
      { name } as unknown as FilterQuery<DomainDBModel> & EnforceEnvOrOrgIds,
      [
        "_id",
        "name",
        "status",
  ...
  async listDomains({
      ...
      domain = await this.findOne(
        {
          ...
          _id: id,
          ...(name
            ? { name: { $regex: this.regExpEscape(name), $options: "i" } }
            : {}),
        },
    ...
    const afterCursor =
      after && domain
        ? { sortBy: domain[sortBy], paginateField: domain._id }
        : undefined
    const beforeCursor =
      before && domain
        ? { sortBy: domain[sortBy], paginateField: domain._id }
