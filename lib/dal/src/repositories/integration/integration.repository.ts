...
export type IntegrationQuery = FilterQuery<IntegrationDBModel> &
  EnforceEnvOrOrgIds
...
> {
  ...
  async findHighestPriorityIntegration({
    ...
    IntegrationEntity,
    "_environmentId" | "_organizationId" | "channel"
  >) {
  ...
  async delete(
    ...
    const q = this.integration.delete({
      _id: query._id,
  ...
  async deleteMany(query: IntegrationQuery): Promise<IDeleteResult> {
    ...
    const { acknowledged, modifiedCount, matchedCount } =
      await this.integration.delete(query)
