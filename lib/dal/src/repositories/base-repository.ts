...
export class BaseRepository<T_DBModel, T_MappedEntity, T_Enforcement> {
  ...
  async findOne(
    ...
    options: {
      ...
      enhanceQuery?: <
        TQuery extends QueryWithHelpers<
          T_DBModel | null,
          T_DBModel,
          {},
          T_DBModel,
    ...
    let queryBuilder = this.MongooseModel.findOne(
      query,
      select,
      queryOptions.query
  ...
  private async createCursorBasedOrStatement({
    ...
    return enhancedCursorOrStatements.length > 0
      ? enhancedCursorOrStatements
      : cursorOrStatements
  }
  ...
  async upsertMany(data: (FilterQuery<T_DBModel> & T_Enforcement)[]) {
    const promises = data.map((entry) =>
      this.MongooseModel.findOneAndUpdate(entry, entry, {
        upsert: true,
        new: true,
      })
    )
