...
> {
  ...
  public async storeJobs(
    ...
    for (let index = 0; index < jobs.length; index += 1) {
      ...
      const created = new this.MongooseModel({
        ...jobs[index],
        createdAt: Date.now(),
      })
  ...
  public async getExistingDelayedJobWithTheSameDigestValue(
    job: JobEntity,
    ...
    return findOne != null
      ? { _id: findOne._id, _notificationId: findOne._notificationId }
      : null
  }
  ...
  public async getAnotherJobTriggeredWithinBackoffTime(
    ...
    const otherDigestJobsWithSameDigestKeyValue = await this.find(
      this.buildLookBackDigestQuery(metadata, job)
    )
    ...
    return await this.find({
      ...
      transactionId: {
        $in: otherDigestJobsWithSameDigestKeyValue.map(
          (job1) => job1.transactionId
        ),
      },
    })
  }
  ...
  async updateAllChildJobStatus(
    ...
    let childJob: JobEntity | null =
      await this.MongooseModel.findOneAndUpdate<JobEntity>(
        {
          _environmentId: job._environmentId,
          _parentId: job._id,
        },
