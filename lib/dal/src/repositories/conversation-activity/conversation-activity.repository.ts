...
import {
  ...
  type ConversationActivitySignalData,
...
function resolveListActivitiesSortBy(sortBy?: string): ListActivitiesSortField {
  if (
    sortBy &&
    (LIST_ACTIVITIES_SORT_FIELDS as readonly string[]).includes(sortBy)
  ) {
...
> {
  ...
  async findByConversation(
    ...
    return this.find(
      { _environmentId: environmentId, _conversationId: conversationId },
      "*",
      {
        sort: { createdAt: -1 },
        limit,
  ...
  async listActivities({
    ...
    const afterCursor =
      after && activity
        ? { sortBy: activity[validatedSortBy], paginateField: activity._id }
        : undefined
    const beforeCursor =
      before && activity
        ? { sortBy: activity[validatedSortBy], paginateField: activity._id }
