...
> {
  ...
  async findNameAndTriggersByIds(
    ...
    Pick<NotificationTemplateEntity, "name" | "triggers" | "_environmentId">[]
  > {
    return this.find(
      {
        _id: { $in: workflowIds },
  ...
  async findByTriggerIdentifierBulk<K extends keyof NotificationTemplateEntity>(
    ...
    NotificationTemplateEntity[] | Pick<NotificationTemplateEntity, K>[]
  > {
    ...
    const baseQuery = this.MongooseModel.find(requestQuery, projection, {
      session,
    })
    const query =
      !select || select.includes("steps" as K)
        ? baseQuery.populate("steps.template")
        : baseQuery

    const items = await query

    return this.mapEntities(items) as
      | NotificationTemplateEntity[]
      | Pick<NotificationTemplateEntity, K>[]
  }
  ...
  async getTotalSteps(): Promise<number> {
    const res = await this.notificationTemplate.aggregate<{
      totalSteps: number
    }>([
      {
        $group: {
