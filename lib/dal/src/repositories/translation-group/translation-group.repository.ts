...
> {
  ...
  public async getList(
    ...
    return {
      ...
      data: this.mapEntities(items) as TranslationGroupWithTranslations[],
    }
  }
  ...
  public async findDeleted(
    query: TranslationGroupQuery
  ): Promise<TranslationGroupEntity[]> {
    const res: TranslationGroupEntity[] =
      await this.translationGroup.findDeleted(query)
  ...
  public async getVariables(
    ...
    const items = await this.MongooseModel.find({
      ...
      path: "translations",
      match: { isoLanguage: defaultLocale },
      select: "translations",
    })
