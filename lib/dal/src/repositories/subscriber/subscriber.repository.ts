...
> {
  ...
  async listSubscribers(query: {
    ...
    const after =
      query.after && subscriber
        ? { sortBy: subscriber[query.sortBy], paginateField: subscriber._id }
        : undefined
    const before =
      query.before && subscriber
        ? { sortBy: subscriber[query.sortBy], paginateField: subscriber._id }
        : undefined
...
function pickUpdatableSubscriberFields(
  subscriber: ISubscribersDefine
): Partial<ISubscribersDefine> {
...
function isErrorWithWriteErrors(
  e: unknown
): e is { writeErrors?: any; message?: string; result?: any } {
