...
> {
  ...
  async createSubscriptions(
    subscriptions: CreateTopicSubscribersEntity[]
  ): Promise<BulkAddTopicSubscribersResult> {
    ...
    try {
      ...
      if (isErrorWithWriteErrors(e)) {
        ...
        throw new DalException(
          "An unknown error occurred while adding topic subscribers"
        )
      }
    }
    ...
    const updatedSubscriptionsInput = subscriptions.filter(
      (_, index) => !createdOrFailedIndexes.includes(index)
    )
    ...
    if (updatedSubscriptionsInput.length > 0) {
      for (const subscription of updatedSubscriptionsInput) {
        const { _subscriberId, _topicId, _environmentId, _organizationId } =
          subscription
  ...
  async *getTopicDistinctSubscribers({
    ...
    { _id: string; subscriberId: string; _topicId: string; identifier: string },
    void,
    unknown
  > {
    const {
    ...
    const mappedTopicIds = topicIds.map((id) =>
    ...
    for await (const doc of this._model
      .aggregate(aggregatePipeline, { batchSize })
      .cursor()) {
      yield doc
    }
  }
  ...
  async countSubscriptionsPerSubscriber({
    ...
    const mappedSubscriberIds = subscriberIds.map((id) =>
      this.convertStringToObjectId(id)
    )
...
function isErrorWithWriteErrors(
  e: unknown
): e is { writeErrors?: unknown; message?: string; result?: unknown } {
