...
> {
  ...
  async delete(query: FilterQuery<FeedEntity>) {
    ...
    const relatedMessages =
      await this.messageTemplateRepository.getMessageTemplatesByFeed(
        feed._environmentId,
        feed._id
      )
    if (relatedMessages.length)
      throw new DalException(`Can not delete feed that has existing message`)

    return await this.feed.delete({
      _id: feed._id,
      _environmentId: feed._environmentId,
    })
  }
