...
> {
  ...
  async findOrCreateContextsFromPayload(
    ...
    const findOrCreatePromises = Object.entries(contextPayload).map(
      ([type, value]) => {
        if (!value) return null

        const { id, data } =
          typeof value === "string"
            ? { id: value, data: undefined }
    ...
    const validPromises = findOrCreatePromises.filter(
      (promise): promise is Promise<ContextEntity> => promise !== null
    )
  ...
  async findOrCreateContext(
    ...
    try {
      ...
      const isDuplicateKeyError =
        ...
        typeof error === "object" &&
        "code" in error &&
        error.code === ErrorCodesEnum.DUPLICATE_KEY
