...
export interface FindWithCursorPaginationOptionsV2<
  T_DBModel,
  T_Select = undefined,
> {
...
export class BaseRepositoryV2<T_DBModel, T_MappedEntity, T_Enforcement> {
  ...
  async aggregate(
    ...
    return this.MongooseModel.aggregate(query).read(
      options.readPreference ?? this.defaultReadPreference
    )
  }
  ...
  async findOne<S extends SelectFieldsObject<T_MappedEntity> & { _id: 0 }>(
    ...
    T_MappedEntity,
    Exclude<IncludedKeys<S, T_MappedEntity>, "_id">
  > | null>
  ...
  async findOne<S extends SelectFieldsObject<T_MappedEntity>>(
    ...
    T_MappedEntity,
    | IncludedKeys<S, T_MappedEntity>
    | ("_id" extends keyof T_MappedEntity ? "_id" : never)
  > | null>
  ...
  async findById(
    ...
    return this.findOne(
      query as unknown as FilterQuery<T_DBModel> & T_Enforcement,
      select as any,
      options
    )
  }
  ...
  async find<S extends SelectFieldsObject<T_MappedEntity> & { _id: 0 }>(
    ...
    Pick<T_MappedEntity, Exclude<IncludedKeys<S, T_MappedEntity>, "_id">>[]
  >
  ...
  async find(
    ...
    const builder = this.MongooseModel.find(query, projection, {
      sort: sort ?? null,
    })
      .skip(skip as number)
  ...
  async create(
    ...
    const mongooseOptions = saveOptions?.writeConcern
      ? { w: saveOptions.writeConcern }
      : {}
  ...
  async bulkWrite(
    bulkOperations: mongo.AnyBulkWriteOperation[],
    ordered = false
  ): Promise<any> {
  ...
  async findWithCursorBasedPagination<
    S extends SelectFieldsObject<T_MappedEntity> & { _id: 0 },
  >(
  ...
  async findWithCursorBasedPagination<
    S extends SelectFieldsObject<T_MappedEntity>,
  >(
    options: FindWithCursorPaginationOptionsV2<T_DBModel, S> & {
      query?: FilterQuery<T_DBModel> & T_Enforcement
    }
  ...
  async findWithCursorBasedPagination({
    ...
    includeCursor,
    select,
  }: FindWithCursorPaginationOptionsV2<
    T_DBModel,
    SelectInput<T_MappedEntity> | "*" | undefined
  > & {
    query?: FilterQuery<T_DBModel> & T_Enforcement
  }): Promise<any> {
    if (before && after) {
      throw new DalException(
        'Cannot specify both "before" and "after" cursors at the same time.'
      )
    }
    ...
    const [rawResults, countResult] = await Promise.all([
      builder.exec(),
      this.getCountWithLimit(query, 50001),
    ])
      ...
      nextQuery.$or = [
        ...
        },
        {
          $and: [
            ...
            {
              [paginateField]: isDesc
                ? { $lt: lastItem[paginateField] }
                : { $gt: lastItem[paginateField] },
            },
          ],
        },
      ]
      ...
      prevQuery.$or = [
        ...
        },
        {
          $and: [
            ...
            {
              [paginateField]: isDesc
                ? { $gt: firstItem[paginateField] }
                : { $lt: firstItem[paginateField] },
            },
          ],
        },
      ]
