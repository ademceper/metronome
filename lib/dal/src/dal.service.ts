...
export class DalService {
  ...
  async connect(url: string, config: ConnectOptions = {}) {
    const baseConfig: ConnectOptions = {
      ...
      maxIdleTimeMS: process.env.MONGO_MAX_IDLE_TIME_IN_MS
        ? Number(process.env.MONGO_MAX_IDLE_TIME_IN_MS)
        : 1000 * 30,
      ...
      minPoolSize: process.env.MONGO_MIN_POOL_SIZE
        ...
        : 10,
      authMechanism:
        (process.env.MONGO_AUTH_MECHANISM as AuthMechanism) ||
        ("DEFAULT" as AuthMechanism),
    }
    ...
    mongoose.connection.on("connected", () =>
      Logger.debug("[@novu/dal]: Mongo connected", MONGODB_CONTEXT)
    )
    mongoose.connection.on("disconnected", () =>
      Logger.debug("[@novu/dal]: Mongo disconnected", MONGODB_CONTEXT)
    )
    mongoose.connection.on("error", (err) =>
      Logger.error(
        `[@novu/dal]: Mongo error: ${err.message}`,
        MONGODB_CONTEXT,
        {
          cause: err,
          stack: err.stack,
