...
> {
  return new APIPromise(
    $do(client, createChannelConnectionRequestDto, idempotencyKey, options)
  )
}
...
> {
  const input: operations.ChannelConnectionsControllerCreateChannelConnectionRequest =
    {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.ChannelConnectionsControllerCreateChannelConnectionRequest$outboundSchema.parse(
        value
      ),
  ...
  >(
    M.json(
      201,
      operations.ChannelConnectionsControllerCreateChannelConnectionResponse$inboundSchema,
