...
function MenubarCheckboxItem({
  ...
  return (
    <MenubarPrimitive.CheckboxItem
      ...
      data-inset={inset}
      className={cn(
        "relative flex cursor-default select-none items-center gap-1.5 rounded-md py-1 ps-7 pe-1.5 text-sm outline-hidden focus:bg-accent focus:text-accent-foreground focus:**:text-accent-foreground data-disabled:pointer-events-none data-inset:ps-7 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
      checked={checked}
      {...props}
...
function MenubarRadioItem({
  ...
  return (
    <MenubarPrimitive.RadioItem
      ...
      data-inset={inset}
      className={cn(
        "relative flex cursor-default select-none items-center gap-1.5 rounded-md py-1 ps-7 pe-1.5 text-sm outline-hidden focus:bg-accent focus:text-accent-foreground focus:**:text-accent-foreground data-disabled:pointer-events-none data-inset:ps-7 data-disabled:opacity-50 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
      {...props}
    >
...
function MenubarLabel({
  ...
  return (
    <MenubarPrimitive.Label
      ...
      data-inset={inset}
      className={cn(
        "px-1.5 py-1 font-medium text-sm data-inset:ps-7",
        className
      )}
      {...props}
    />
...
function MenubarSubTrigger({
  ...
  return (
    <MenubarPrimitive.SubTrigger
      ...
      data-inset={inset}
      className={cn(
        "flex cursor-default select-none items-center gap-1.5 rounded-md px-1.5 py-1 text-sm outline-none focus:bg-accent focus:text-accent-foreground data-open:bg-accent data-inset:ps-7 data-open:text-accent-foreground [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
      {children}
      <CaretRightIcon className="ms-auto size-4 rtl:rotate-180" />
    </MenubarPrimitive.SubTrigger>
  )
}
