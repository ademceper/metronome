...
> {
  ...
  async findOne(
    ...
    const queryBuilder = this.MongooseModel.findOne(
      query,
      select,
      queryOptions.query
    )
  ...
  async createLayout(
    entity: Omit<LayoutEntity, "_id" | "createdAt" | "updatedAt">
  ): Promise<LayoutEntity> {
    const {
      channel,
      content,
  ...
  async filterLayouts(
    ...
    parsedQuery._environmentId = this.convertStringToObjectId(
      parsedQuery._environmentId
    )
    parsedQuery._organizationId = this.convertStringToObjectId(
      parsedQuery._organizationId
    )
  ...
  async updateLayout(entity: LayoutEntity): Promise<LayoutEntity> {
    ...
    if (updated.matched === 0 || updated.modified === 0) {
      throw new DalException(
        `Update of layout ${_id} in environment ${_environmentId} was not performed properly`
      )
    }
  ...
  private async paginate(
    ...
    const items = await this.MongooseModel.find({
      ...
      .sort({
        [pagination.orderBy]:
          pagination.orderDirection === DirectionEnum.ASC ? 1 : -1,
      })
      .skip(pagination.skip)
      .limit(pagination.limit)
      .lean()
