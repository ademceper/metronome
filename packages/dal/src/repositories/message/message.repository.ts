...
function mergeTagsMongoFragment<
  MessageQueryT extends MessageQuery & EnforceEnvId,
>(query: MessageQueryT, fragment: TagsMongoFragment): MessageQueryT {
...
> {
  ...
  private chunkArray<T>(
    array: T[],
    size: number = MessageRepository.BATCH_SIZE
  ): T[][] {
    const chunks: T[][] = []
  ...
  async findOne(
    ...
    const transformedQuery = this.transformContextKeysQuery(
      query
    ) as FilterQuery<MessageDBModel> & EnforceEnvId
  ...
  private async getFilterQueryForMessage(
    ...
    if (query.tagGroups && query.tagGroups.length > 0) {
      requestQuery = mergeTagsMongoFragment(
        requestQuery,
        buildTagsQuery(query.tagGroups)
      )
    }
    ...
    return await this.cursorPagination({
      ...
      enhanceQuery: (queryBuilder) =>
        queryBuilder
          ...
          .populate(
            "actorSubscriber",
            "_id firstName lastName avatar subscriberId"
          )
          .populate({
            path: "template",
            select: "_id name tags data critical triggers severity",
  ...
  async getCount(
    ...
    return this.MongooseModel.countDocuments(requestQuery, options).read(
      readPreference
    )
  }
  ...
  async markAllMessagesAs({
    ...
    const updateQuery = this.getReadSeenUpdateQuery(
      subscriberId,
      environmentId,
      markAs
    )
  ...
  async updateFeedByMessageTemplateId(
    environmentId: string,
    messageId: string,
    feedId?: string | null
  ) {
  ...
  async updateMessagesStatusByIds({
    ...
    const query: MessageQuery & EnforceEnvId = {
      ...
      _subscriberId: subscriberId,
      ...(contextKeys &&
        contextKeys?.length > 0 && { contextKeys: { $in: contextKeys } }),
      _id: {
        $in: ids.map((id) => {
  ...
  async findWithSubscriber(
    ...
    const res = await this.MongooseModel.find(query, select)
      .populate("subscriber", "subscriberId")
      .lean()
      .exec()
  ...
  private transformContextKeysQuery(
    query: FilterQuery<MessageDBModel>
  ): FilterQuery<MessageDBModel> {
