 ...
 */
export function domainsDiagnose(
  client: NovuCore,
  domain: string,
  ...
  >
> {
  return new APIPromise($do(client, domain, idempotencyKey, options))
}
...
> {
  const input: operations.DomainsControllerDiagnoseDomainRequest = {
    domain: domain,
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.DomainsControllerDiagnoseDomainRequest$outboundSchema.parse(
        value
      ),
  ...
  const path = pathToFunc("/v1/domains/{domain}/diagnose")(pathParams)
  ...
  >(
    M.json(
      200,
      operations.DomainsControllerDiagnoseDomainResponse$inboundSchema,
