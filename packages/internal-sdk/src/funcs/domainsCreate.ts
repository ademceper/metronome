...
export function domainsCreate(
  ...
  >
> {
  return new APIPromise($do(client, createDomainDto, idempotencyKey, options))
}
...
> {
  const input: operations.DomainsControllerCreateDomainRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.DomainsControllerCreateDomainRequest$outboundSchema.parse(
        value
      ),
  ...
  const body = encodeJSON("body", payload.CreateDomainDto, { explode: true })

  const path = pathToFunc("/v1/domains")()
  ...
  >(
    M.json(
      201,
      operations.DomainsControllerCreateDomainResponse$inboundSchema,
