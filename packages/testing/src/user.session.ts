...
export class UserSession {
  ...
  private async initializeCommunity(options: UserSessionOptions = {}) {
    ...
    const userEntity: Partial<UserEntity> = {
      ...
      email:
        `${card.firstName}_${card.lastName}_${faker.datatype.uuid()}@gmail.com`.toLowerCase(),
    ...
    if (
      ...
      !options.noEnvironment &&
      !options.noWidgetSession
    ) {
  ...
  private get requestEndpoint() {
    return this.shouldUseTestServer()
      ? this.testServer?.getHttpServer()
      : this.serverUrl
  }
  ...
  async updateEETokenClaims(claims: Partial<ClerkJwtPayload>) {
    try {
      const currentPayload = this.token
        ? jwt.decode(this.token.replace("Bearer ", ""))
        : null

      const baseToken = process.env.CLERK_LONG_LIVED_TOKEN as string
      const decodedBaseToken = jwt.decode(baseToken)
      const payload = {
        ...(typeof decodedBaseToken === "object" && decodedBaseToken !== null
          ? decodedBaseToken
          : {}),
      ...
      if (!privateKey) {
        throw new Error(
          "CLERK_MOCK_JWT_PRIVATE_KEY environment variable is not set"
        )
      }
  ...
  async createIntegrations(environments: EnvironmentEntity[]): Promise<void> {
    for (const environment of environments) {
      await this.integrationService.createChannelIntegrations(
        environment._id,
        this.organization._id
      )
    }
  ...
  async switchToProdEnvironment() {
    const prodEnvironment =
      await this.environmentService.getProductionEnvironment(
        this.organization._id
      )
    if (prodEnvironment) {
  ...
  async switchToDevEnvironment() {
    const devEnvironment =
      await this.environmentService.getDevelopmentEnvironment(
        this.organization._id
      )
    if (devEnvironment) {
  ...
  public async waitForDbJobCompletion({
    ...
    return this.jobsService.waitForDbJobCompletion({
      templateId,
      organizationId,
      maxWaitTime,
    })
  ...
  public async updateOrganizationServiceLevel(
  ...
  public async updateEnvironmentApiRateLimits(
    apiRateLimits: Partial<IApiRateLimitMaximum>
  ) {
    await this.environmentService.updateApiRateLimits(
      this.environment._id,
      apiRateLimits
    )
  }
