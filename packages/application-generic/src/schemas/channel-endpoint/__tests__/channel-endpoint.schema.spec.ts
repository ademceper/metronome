...
describe("ChannelEndpointSchema", () => {
  ...
  it("should validate endpoints correctly", () => {
    // Valid cases
    expect(
      validateEndpointForTypeFromSchema(ENDPOINT_TYPES.SLACK_CHANNEL, {
        channelId: "C123",
      })
    ).toBe(true)
    expect(
      validateEndpointForTypeFromSchema(ENDPOINT_TYPES.SLACK_USER, {
        userId: "U123",
      })
    ).toBe(true)
    expect(
      validateEndpointForTypeFromSchema(ENDPOINT_TYPES.WEBHOOK, {
