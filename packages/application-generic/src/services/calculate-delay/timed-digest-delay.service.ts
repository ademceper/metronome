...
export class TimedDigestDelayService {
  ...
  public static calculate({
    ...
    const currentTimeTz = timezone
      ? toZonedTime(new Date(), timezone)
      : new Date()

    const { bysetpos, byweekday, bymonthday } =
      TimedDigestDelayService.calculateByFields({
        weekDays,
        monthDays,
        monthlyType,
        ordinal,
    ...
    const rule = new RRule({
      dtstart: dateStartTz,
      until: TimedDigestDelayService.getUntilDate(
        dateStartTz,
        unit,
        amount,
        timezone
    ...
    const currentUtc = timezone
      ? fromZonedTime(currentTimeTz, timezone)
      : currentTimeTz
