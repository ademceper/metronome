...
describe("SqsPayloadOffloadService", () => {
  ...
  describe("maybeOffload", () => {
    ...
    it("should upload to S3 and return reference when over threshold", async () => {
      ...
      expect(putCall.input.Key).toMatch(
        /^sqs-payloads\/standard\/org-123\/\d{4}-\d{2}-\d{2}\/msg-1-.+\.json$/
      )
      ...
      expect(parsed.__sqsLargePayload.key).toMatch(
        /^sqs-payloads\/standard\/org-123\//
      )
    })
  ...
  describe("maybeResolve", () => {
    ...
    it("should fetch from S3 when body is an S3 reference", async () => {
      ...
      expect(getCall.input.Key).toBe(
        "sqs-payloads/standard/org-123/2026-03-17/msg-1-abc123.json"
      )
    })
    ...
    it("should return body as-is when reference has invalid structure", async () => {
      ...
      const invalid = JSON.stringify({
        __sqsLargePayload: { bucket: 123, key: null },
      })
  ...
  describe("round-trip", () => {
    it("should offload and resolve back to the original payload", async () => {
      ...
      const offloaded = await service.maybeOffload(
        originalBody,
        "workflow",
        "job-0",
        "org-456"
  ...
  describe("constructor", () => {
    it("should pass endpoint and forcePathStyle for local development", () => {
      const service = new SqsPayloadOffloadService(
        "us-east-1",
        "http://localhost:4566"
      )
