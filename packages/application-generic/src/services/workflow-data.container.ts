...
export class WorkflowDataContainer {
  ...
  async loadWorkflowsWithControlValues(
    ...
    const [controlValues, preferences] = await this.fetchRelatedData(
      Array.from(lookupMaps.workflowLookup.values()).map(
        (data) => data.objectId
      ),
      environmentIds,
      organizationId
    ...
    const controlValuesByWorkflowAndStep = this.organizeControlValues(
      controlValues,
      lookupMaps.objectIdToKey
    )
    const preferencesByWorkflow = this.organizePreferences(
      preferences,
      lookupMaps.objectIdToKey
    )
  ...
  private organizeControlValues(
    controlValues: ControlValuesEntity[],
    objectIdToKey: Map<string, string>
  ) {
    const byWorkflowAndStep = new Map<
      string,
      Map<string, ControlValuesEntity>
    >()

    for (const cv of controlValues) {
      if (!cv._workflowId || !cv._stepId) continue

      const lookupKey = objectIdToKey.get(
        `${cv._workflowId}:${cv._environmentId}`
      )
      if (!lookupKey) continue

      this.ensureMapEntry(byWorkflowAndStep, lookupKey, new Map()).set(
        cv._stepId,
        cv
      )
    }
  ...
  private organizePreferences(
    ...
    for (const pref of preferences) {
      ...
      const lookupKey = objectIdToKey.get(
        `${pref._templateId}:${pref._environmentId}`
      )
      if (!lookupKey) continue
  ...
  private processWorkflows(
    workflows: NotificationTemplateEntity[],
    controlValuesByWorkflowAndStep: Map<
      string,
      Map<string, ControlValuesEntity>
    >,
    ...
    for (const workflow of workflows) {
      ...
      const controlValuesByStep =
        controlValuesByWorkflowAndStep.get(key) || new Map()
      const preferences = preferencesByWorkflow.get(key)

      const workflowWithPreferences = this.buildWorkflowWithPreferences(
        workflow,
        preferences
      )
  ...
  private buildWorkflowWithPreferences(
    workflow: NotificationTemplateEntity,
    preferences?: IWorkflowPreferences
  ) {
    ...
    const defaultPreferences = preferences?.workflowResourcePreference
      ?.preferences
      ? buildWorkflowPreferences(
          preferences.workflowResourcePreference.preferences
        )
  ...
  getWorkflowData(
    ...
    const data = this.workflowsByIdentifier.get(
      this.makeKey(environmentId, identifier)
    )
    if (data) {
      return data
    }
    ...
    for (const workflowData of this.workflowsByIdentifier.values()) {
      if (
        workflowData.workflow._id === identifier &&
        workflowData.workflow._environmentId === environmentId
      ) {
        return workflowData
      }
    }
