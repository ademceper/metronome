...
export class SocketWorkerService {
  ...
  private async handleReceivedEvent({
    ...
    if (!storedMessage) {
      Logger.error(
        `Message with id ${messageId} not found in environment ${environmentId}`,
        LOG_CONTEXT
      )
  ...
  private async sendMessageInternal({
    ...
    if (!this.socketWorkerUrl) {
      Logger.debug(
        "Socket worker URL not configured, skipping dispatch",
        LOG_CONTEXT
      )
    ...
    if (!this.socketWorkerApiKey) {
      Logger.error(
        "Socket worker API key not configured, cannot dispatch",
        LOG_CONTEXT
      )
    ...
    try {
      ...
      Logger.debug(
        `Dispatching event ${event} to socket worker for user ${userId}`,
        LOG_CONTEXT
      )
      ...
      Logger.debug(
        `Successfully dispatched event ${event} to socket worker for user ${userId}`,
        LOG_CONTEXT
      )
    } catch (error) {
      if (error instanceof HTTPError) {
        ...
        if (statusCode === 401) {
          ...
          Logger.error(
            `Failed to dispatch to socket worker: ${statusCode} - ${errorText}`,
            LOG_CONTEXT
          )
        }
      } else if (error instanceof RequestError) {
        Logger.error(
          `Request error dispatching to socket worker: ${error.message}`,
          LOG_CONTEXT
        )
      } else {
        Logger.error(
  ...
  async sendUnseenCount(
    ...
    return this.sendUnseenCountChange(
      userId,
      environmentId,
      contextKeys,
      organizationId
  ...
  async sendUnreadCount(
    ...
    return this.sendUnreadCountChange(
      userId,
      environmentId,
      contextKeys,
      organizationId
