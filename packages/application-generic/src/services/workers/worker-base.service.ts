...
export type SqsCompletedHandler = (
...
export class WorkerBaseService implements INovuWorker, OnModuleDestroy {
  ...
  public initWorker(
    ...
    Logger.log(
      { topic: this.topic, sqsEnabled: !!this.sqsConsumer },
      "Worker initialized",
      LOG_CONTEXT
    )
  }
  ...
  private wrapForBullMQ(
    processor: Processor<any, unknown, string>
  ): Processor<any, unknown, string> {
  ...
  private initSqsConsumer(
    ...
    const sqsConsumerOptions: ISqsConsumerOptions = {
      ...
      waitTimeSeconds:
        getSqsDefaultWaitTimeSeconds() ?? SQS_DEFAULT_WAIT_TIME_SECONDS,
      visibilityTimeout:
        getSqsDefaultVisibilityTimeout() ?? SQS_DEFAULT_VISIBILITY_TIMEOUT,
  ...
  private wrapForSqs(
    processor: Processor<any, unknown, string>
  ): (data: any, meta: ISqsMessageMeta) => Promise<void> {
  ...
  public async gracefulShutdown(): Promise<void> {
    ...
    const shutdownPromises: Promise<void>[] = [
      this.bullMqService.gracefulShutdown(),
    ]

    if (this.sqsConsumer) {
      shutdownPromises.push(
        this.sqsConsumer.stop({ drainTimeoutMs: SQS_DEFAULT_DRAIN_TIMEOUT_MS })
      )
    }
    ...
    Logger.log(
      { topic: this.topic },
      "Worker service shutdown complete",
      LOG_CONTEXT
    )
  }
