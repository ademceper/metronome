...
describe("MsTeamsTokenService", () => {
  ...
  describe("getGraphToken", () => {
    it("should request a client_credentials token with the Graph scope", async () => {
      axiosPost.resolves({
        data: { access_token: MOCK_ACCESS_TOKEN, expires_in: 3600 },
      })

      const token = await service.getGraphToken(
        MOCK_CLIENT_ID,
        MOCK_SECRET_KEY,
        MOCK_TENANT_ID
      )
      ...
      expect(params.get("scope")).to.equal(
  ...
  describe("getBotFrameworkToken", () => {
    it("should request a client_credentials token with the Bot Framework scope", async () => {
      ...
      const token = await service.getBotFrameworkToken(
        MOCK_CLIENT_ID,
        MOCK_SECRET_KEY,
        MOCK_TENANT_ID
      )
