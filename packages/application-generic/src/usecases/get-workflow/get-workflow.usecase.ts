...
export class GetWorkflowUseCase {
  ...
  async execute(
    ...
    if (workflowDataContainer) {
      ...
      if (cachedDto) {
        this.logger.debug(
          `Using cached workflow DTO for ${command.workflowIdOrInternalId}`
        )
  ...
  private async resolveEnvironmentId(
    ...
    const environment =
      await this.environmentRepository.findByIdAndOrganization(
        environmentId,
  ...
  private async buildStepForWorkflow(
    ...
    try {
      ...
      return {
        ...stepResponse,
        issues:
          Object.keys(combinedIssues).length > 0 ? combinedIssues : undefined,
      }
    } catch (error) {
  ...
  private validateIntegrationFromCache(
    ...
    const primaryNeeded =
      stepType === StepTypeEnum.EMAIL || stepType === StepTypeEnum.SMS

    // Find the relevant integration from the pre-fetched list
    const validIntegrationForStep = availableIntegrations.find(
      (integration) => {
        const matchesChannel = integration.channel === channelType
        const matchesPrimary = primaryNeeded
