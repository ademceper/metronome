...
export class VerifyPayload {
  ...
  execute(command: VerifyPayloadCommand): Record<string, unknown> {
    ...
    for (const step of command.template.steps) {
      invalidKeys.push(
        ...verifyPayloadService.checkRequired(
      ...
      if (
        ...
        if (!("delayPath" in step.metadata) || !step.metadata.delayPath) {
          throw new BadRequestException(
            "Delay path is required for scheduled delay"
          )
        }

        const invalidKey = this.checkRequiredDelayPath(
          step.metadata.delayPath,
          command.payload
        )
        if (invalidKey) {
    ...
    for (const step of command.template.steps) {
      defaultPayload = verifyPayloadService.fillDefaults(
        step.template?.variables || []
      )
    }
  ...
  private checkRequiredDelayPath(
    delayPath: string,
