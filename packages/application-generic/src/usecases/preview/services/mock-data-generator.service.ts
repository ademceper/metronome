...
export class MockDataGeneratorService {
   ...
   */
  generateMockStepResult(
    options: MockStepResultOptions
    ...
    try {
      ...
      if (stepType in channelStepSchemas) {
        resultSchema =
          channelStepSchemas[stepType as keyof typeof channelStepSchemas].result
      } else if (stepType in actionStepSchemas) {
        resultSchema =
          actionStepSchemas[stepType as keyof typeof actionStepSchemas].result
      }
  ...
  private generateHttpRequestStepResult(
    ...
    if (
      ...
      const properties = responseBodySchema.properties as Record<
        string,
        unknown
      >
      if (Object.keys(properties).length > 0) {
        return JsonSchemaMock.generate(responseBodySchema) as Record<
          string,
          unknown
        >
      }
  ...
  private generateDigestStepResult(
    ...
    try {
      ...
      if (workflow?.payloadSchema) {
        payloadMockData = JsonSchemaMock.generate(
