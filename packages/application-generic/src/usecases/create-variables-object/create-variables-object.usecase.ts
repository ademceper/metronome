...
export class CreateVariablesObject {
  ...
  async execute(
    ...
    const showIfVariables = this.extractMailyAttribute(
      command.controlValues,
      MailyAttrsEnum.SHOW_IF_KEY
    )

    const variablesObject = keysToObject(
      variables,
      arrayVariables,
      showIfVariables
    )
  ...
  private async ensureEventsVariableIsAnArray(
    ...
    Object.keys(stepsObject).forEach((stepId) => {
      ...
      const hasUsedEvents =
        !!(step.events && typeof step.events === "string") ||
        Array.isArray(step.events)
      /**
      ...
      if (
        ...
        if (isPayloadSchemaEnabled && command.payloadSchema) {
          try {
            ...
            payload = this.generateFallbackPayload(
              step,
              hasUsedEventsWithPayload
            )
          }
  ...
  private generateFallbackPayload(
    ...
    if (Array.isArray(step.events)) {
      ...
      const variableNameAfterPayload = collectKeys(
        (step.events as Record<string, unknown>).payload
      )
      for (const variableName of variableNameAfterPayload) {
        ...
        payload = {
          ...payload,
          ...this.setNestedValue(payload, variableName, key),
        }
      }
    }
  ...
  private extractArrayVariables(controlValues: unknown[]): ArrayVariable[] {
    ...
    const eachKeyVars = this.extractMailyAttribute(
      controlValues,
    ...
    const idVars = this.extractMailyAttribute(
      controlValues,
      MailyAttrsEnum.ID,
      this.extractArrayPath
    ).map((path) => ({
      path,
      iterations: DEFAULT_ARRAY_ELEMENTS,
    }))
