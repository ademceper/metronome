...
describe("GetLayoutUseCase", () => {
  ...
  beforeEach(() => {
    ...
    controlValuesRepositoryMock = sinon.createStubInstance(
      ControlValuesRepository
    )
    layoutVariablesSchemaUseCaseMock = sinon.createStubInstance(
    ...
    layoutVariablesSchemaUseCaseMock.execute.resolves(
      mockVariablesSchema as any
    )
  })
  ...
  describe("execute", () => {
    it("should successfully get layout with control values", async () => {
      ...
      expect(result.controls.values?.email).to.deep.equal(
        mockControlValues.controls.email
      )
    ...
    it("should get layout without control values when none exist", async () => {
      ...
      expect(
        controlValuesRepositoryMock.findOne.firstCall.args[0]
      ).to.deep.equal({
        _environmentId: "env_id",
        _organizationId: "org_id",
        _layoutId: "layout_id",
    ...
    it("should call layout variables schema use case", async () => {
      ...
      const schemaCommand =
        layoutVariablesSchemaUseCaseMock.execute.firstCall.args[0]
    ...
    it("should handle empty control values controls", async () => {
      ...
      controlValuesRepositoryMock.findOne.resolves(
        controlValuesWithEmptyControls as any
      )
