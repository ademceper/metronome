...
describe("extractLiquidTemplateVariables", () => {
  ...
  describe("Basic output variables without schema", () => {
    it("should extract simple variables", () => {
      ...
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
  ...
  describe("Basic output variables with schema", () => {
    ...
    it("should extract simple variables", () => {
      ...
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template, variableSchema })
  ...
  describe("Variables with filters", () => {
    ...
    it("should handle toSentence filter with arguments", () => {
      ...
      expect(validVariables[0].name).to.equal(
        "steps.digest-step.events.payload.name"
      )
  ...
  describe("For loops", () => {
    it("should handle for loops with valid collection variable", () => {
      const template =
        "{% for item in payload.items %}{{item.name}}{{invalid}}{% endfor %}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
    ...
    it("should handle for loops with invalid collection variable", () => {
      const template =
        "{% for item in invalidCollection %}{{item.name}}{{payload.foo}}{% endfor %}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
    ...
    it("should handle for loops with ranges (variables)", () => {
      const template =
        "{% for i in (payload.start..payload.end) %}{{i}}{{payload.foo}}{{test}}{% endfor %}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
    ...
    it("should handle nested for loops", () => {
      ...
      const variableNames = validVariables.map((variable) => variable.name)
      const invalidVariableNames = invalidVariables.map(
  ...
  describe("For loop collection type inference", () => {
    it("should mark collection variable as for-loop collection with iterator properties", () => {
      const template =
        "{% for provider in payload.providers %}{{ provider.label }} ({{ provider.id }}){% endfor %}"
      const { validVariables } = extractLiquidTemplateVariables({ template })

      const collectionVar = validVariables.find(
        (v) => v.name === "payload.providers"
      )
    ...
    it("should handle deep property access on iterator", () => {
      const template =
        "{% for user in payload.users %}{{ user.address.city }}{% endfor %}"
      const { validVariables } = extractLiquidTemplateVariables({ template })

      const collectionVar = validVariables.find(
        (v) => v.name === "payload.users"
      )
      expect(collectionVar).to.exist
      expect(collectionVar!.isForLoopCollection).to.equal(true)
    ...
    it("should handle tablerow with iterator properties", () => {
      ...
      const collectionVar = validVariables.find(
        (v) => v.name === "payload.products"
      )
      expect(collectionVar).to.exist
      expect(collectionVar!.isForLoopCollection).to.equal(true)
      expect(collectionVar!.iteratorProperties).to.have.members([
        "name",
        "price",
      ])
    })
    ...
    it("should annotate collection on invalidVariables when collection is invalid", () => {
      const template =
        "{% for item in invalidCollection %}{{ item.name }}{% endfor %}"
      const { invalidVariables } = extractLiquidTemplateVariables({ template })

      const collectionVar = invalidVariables.find(
        (v) => v.name === "invalidCollection"
      )
      expect(collectionVar).to.exist
      expect(collectionVar!.isForLoopCollection).to.equal(true)
  ...
  describe("Conditional tags", () => {
    ...
    it("should handle unless statements with valid condition", () => {
      const template =
        "{% unless payload.banned %}Show content{{invalid}}{% endunless %}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
    ...
    it("should handle unless statements with invalid condition", () => {
      const template =
        "{% unless user.banned %}Show content{{invalid}}{% endunless %}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
      const invalidVariableNames = invalidVariables.map(
        (variable) => variable.name
  ...
  describe("Assign tags", () => {
    it("should handle assign statements with valid variable", () => {
      const template =
        "{% assign fullName = payload.firstName %}{{fullName}}{{invalid}}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
    ...
    it("should handle assign statements with invalid variable", () => {
      const template =
        "{% assign fullName = user.firstName %}{{fullName}}{{payload.foo}}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
    ...
    it("should handle assign statements with filters without treating filter names as variables", () => {
      const template =
        "{% assign uppercaseName = payload.firstName | upcase %}{{uppercaseName}}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
    ...
    it("should handle assign statements with multiple filters", () => {
      const template =
        "{% assign processedName = payload.firstName | upcase | truncate: 10 %}{{processedName}}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
    ...
    it("should handle assign statements with whitespace control - both sides", () => {
      const template =
        "{%- assign first_name = payload.first_name -%} Hello {{first_name}}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
  ...
  describe("Tablerow tags", () => {
    ...
    it("should handle tablerow with ranges", () => {
      const template =
        "{% tablerow i in (1..payload.count) %}{{i}}{{invalid}}{{payload.foo}}{% endtablerow %}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
  ...
  describe("Edge cases and error handling", () => {
    ...
    it("should deduplicate variables", () => {
      const template =
        "{{user.name}} {{user.name}} {{payload.name}} {{payload.name}}"
      const { validVariables, invalidVariables } =
        extractLiquidTemplateVariables({ template })
  ...
  describe("Complex real-world scenarios", () => {
    ...
    it("should handle undefined filters as invalid", () => {
      ...
      expect(invalidVariables[0].message).to.include(
        "undefined filter: currency"
      )
    })
