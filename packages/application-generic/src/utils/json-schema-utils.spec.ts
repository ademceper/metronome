...
describe("keysToObject", () => {
  ...
  it("should handle array paths correctly", () => {
    const paths = [
      "payload.addresses[1].street",
      "payload.addresses[0].city",
      "payload.addresses[2].street",
    ]
  ...
  it("should handle direct array paths with arrayVariables", () => {
    ...
    const arrayVariables: ArrayVariable[] = [
      { path: "payload.items", iterations: 3 },
    ]
...
describe("mockSchemaDefaults", () => {
  it("should preserve falsy default values (0, false, null, empty string)", () => {
    ...
    expect(result.properties!.insurance_policy_id).to.have.property(
      "default",
      null
    )
  ...
  it("should add template string defaults for properties without defaults", () => {
    ...
    expect(result.properties!.name).to.have.property(
      "default",
      "{{payload.name}}"
    )
    expect(result.properties!.age).to.have.property(
      "default",
      "{{payload.age}}"
    )
  })
