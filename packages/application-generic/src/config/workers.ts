...
function getWorkerConcurrency(
  ...
  const perQueueValue = process.env[envKey]
...
export const getSqsDefaultConcurrency = () =>
  process.env.SQS_DEFAULT_CONCURRENCY
    ? Number(process.env.SQS_DEFAULT_CONCURRENCY)
    : undefined
...
export const getSqsDefaultBatchSize = () => {
  const value = process.env.SQS_DEFAULT_BATCH_SIZE
    ? Number(process.env.SQS_DEFAULT_BATCH_SIZE)
    : undefined
...
export const getInboundParseMailWorkerOptions = () =>
  getWorkerConfig(WorkerEnum.INBOUND_PARSE_MAIL, 200)

export const getSubscriberProcessWorkerOptions = () =>
  getWorkerConfig(WorkerEnum.SUBSCRIBER_PROCESS, 200)

export const getStandardWorkerOptions = () =>
  getWorkerConfig(WorkerEnum.STANDARD, 200)

export const getWebSocketWorkerOptions = () =>
  getWorkerConfig(WorkerEnum.WEB_SOCKET, 400)

export const getWorkflowWorkerOptions = () =>
  getWorkerConfig(WorkerEnum.WORKFLOW, 200)
